...
import { classifyStale, staleThresholdS } from "../src/commands/list.js";
import { mkdtempSync, writeFileSync, utimesSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";

// guards the threshold the list call-site sources from the env
const ORIG = process.env.AP_STALE_THRESHOLD_S;
afterEach(() => { if (ORIG === undefined) delete process.env.AP_STALE_THRESHOLD_S; else process.env.AP_STALE_THRESHOLD_S = ORIG; });
...
describe("staleThresholdS sources the env (AP_STALE_THRESHOLD_S) with shell `:-` parity", () => {
  ...
  it("invalid input (abc/-5/10.5) yields a value classifyStale falls back to 180 for", () => {
    ...
    for (const bad of ["abc", "-5", "10.5"]) {
      process.env.AP_STALE_THRESHOLD_S = bad;
