// tests/implement-turn-cmd.test.ts — B2a: implement turn-send / turn-wait verbs.
...
function sendDeps(over: Partial<ImplementSendDeps> & { record?: (label: string) => void } = {}): ImplementSendDeps {
  return {
...
describe("implement turn-wait (rc 0 always; TS= carries the outcome)", () => {
  let h: { home: string; cleanup: () => void };
  beforeEach(() => { h = freshHome(); });
  afterEach(() => { h.cleanup(); delete process.env.AP_IMPLEMENT_TURN_TIMEOUT_S; });
  ...
  it("AP_IMPLEMENT_TURN_TIMEOUT_S=5 → wait dep receives scaledTimeout(5,'1')===5", async () => {
    seedWait();
    process.env.AP_IMPLEMENT_TURN_TIMEOUT_S = "5";
    let gotTimeout = -1;
