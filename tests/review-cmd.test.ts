// tests/review-cmd.test.ts — survey + archive verbs.
...
describe("review survey", () => {
  ...
  it("lists live forensics as TSV + a TRENDS block; excludes .reviewed/", async () => {
    writeFileSync(join(froot, "11-00-00-implement-add-oauth.md"),
      forensicsDoc("implement", "add-oauth", [["audit_log", "ISSUE=todo_marker", "audit.log"]]));
    ...
    writeFileSync(join(reviewed, "10-00-00-design-old.md"), forensicsDoc("design", "old", [["status", "state=error", "worker=a"]]));
    ...
    expect(rc).toBe(0);
    const t = out.text();
    expect(t).toContain(`${join(froot, "11-00-00-implement-add-oauth.md")}\timplement\tadd-oauth\t1`);
    expect(t).not.toContain("design\told");                 // .reviewed/ excluded by default
  ...
  it("--command filters; --all includes .reviewed/", async () => {
    writeFileSync(join(froot, "11-00-00-implement-x.md"), forensicsDoc("implement", "x", [["status", "state=error", "worker=a"]]));
    const reviewed = join(h.home, "forensics", ".reviewed", "2026-05-29");
    mkdirSync(reviewed, { recursive: true });
    writeFileSync(join(reviewed, "10-00-00-design-y.md"), forensicsDoc("design", "y", [["status", "state=error", "worker=b"]]));
    await surveyWith({ all: true, command: "design" });
    const t = out.text();
    expect(t).toContain("design\ty");                       // --all surfaced the archived file
    expect(t).not.toContain("implement\tx");                 // --command=design filtered out implement
  });
  ...
  it("--since excludes files older than the cutoff (by mtime)", async () => {
    ...
    const oldF = join(froot, "09-00-00-implement-old.md");
    const newF = join(froot, "12-00-00-implement-new.md");
    writeFileSync(oldF, forensicsDoc("implement", "old", [["status", "state=error", "worker=a"]]));
    writeFileSync(newF, forensicsDoc("implement", "new", [["status", "state=error", "worker=b"]]));
    const now = Date.now();
    ...
    const t = out.text();
    expect(t).toContain("implement\tnew");
    expect(t).not.toContain("implement\told");
  });
});
...
describe("review archive", () => {
  ...
  it("accrues the trend and moves files to .reviewed/", async () => {
    const f = join(froot, "11-00-00-implement-x.md");
    writeFileSync(f, forensicsDoc("implement", "x", [["audit_log", "ISSUE=todo_marker", "audit.log"], ["status", "state=error", "worker=a"]]));
    const rc = await archiveWith([f], { now: new Date("2026-05-30T00:00:00Z") });
    expect(rc).toBe(0);
    // file moved
    expect(existsSync(f)).toBe(false);
    expect(existsSync(join(h.home, "forensics", ".reviewed", "2026-05-30", "11-00-00-implement-x.md"))).toBe(true);
    // trend accrued
  ...
  it("accrues onto a seeded ledger: bumps count, advances lastSeen, preserves firstSeen (cross-window trend)", async () => {
    const f = join(froot, "11-00-00-implement-x.md");
    writeFileSync(f, forensicsDoc("implement", "x", [["audit_log", "ISSUE=todo_marker", "audit.log"]]));
    writeFileSync(join(h.home, "forensics", ".trends.json"),
  ...
  it("is idempotent: a path already under .reviewed/ is skipped (no re-accrue)", async () => {
    const reviewed = join(h.home, "forensics", ".reviewed", "2026-05-29");
    mkdirSync(reviewed, { recursive: true });
    const r = join(reviewed, "10-00-00-design-y.md");
    writeFileSync(r, forensicsDoc("design", "y", [["status", "state=error", "worker=b"]]));
  ...
  it("rc 2 when no paths given", async () => {
    const { run } = await import("../src/commands/review.js");
