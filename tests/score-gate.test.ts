...
describe("gateState (pure)", () => {
  it("all workers done with a non-question last line → terminal", () => {
    const out = gateState([
      { agent: "viola", doneExists: true, stateText: "OFFSET=5\nFS=ok\n" },
      { agent: "cello", doneExists: true, stateText: "OFFSET=3\nFS=empty\n" },
    ], "FS");
    expect(out).toEqual([
      { agent: "viola", status: "terminal" },
      { agent: "cello", status: "terminal" },
    ]);
  });

  it("missing .done marker → pending (still running)", () => {
    const out = gateState([
      { agent: "viola", doneExists: true, stateText: "FS=ok\n" },
      { agent: "cello", doneExists: false, stateText: null },
    ], "FS");
  ...
  it("terminal then question — last line wins → question", () => {
    const out = gateState([
      { agent: "cello", doneExists: true, stateText: "FS=ok\nOFFSET=7\nFS=question\n" },
    ], "FS");
    expect(out[0].status).toBe("question");
  });
  ...
  it("verify phase uses the VS= key", () => {
    const out = gateState([
      { agent: "viola", doneExists: true, stateText: "OFFSET=2\nVS=skipped\n" },
      { agent: "cello", doneExists: true, stateText: "OFFSET=4\nVS=question\n" },
    ], "VS");
  ...
  it("verify-style AS key: terminal / question / pending", () => {
    const out = gateState([
      { agent: "viola", doneExists: true, stateText: "OFFSET=2\nAS=ok\n" },
      { agent: "cello", doneExists: true, stateText: "OFFSET=4\nAS=question\n" },
      { agent: "oboe", doneExists: false, stateText: null },
    ], "AS");
