...
describe("parseScoreboard", () => {
  it("picks first-ok winner + next-ok runner-ups (skips partial)", () => {
    ...
    expect(r.winner).toMatchObject({ expId: "exp-003", agent: "violin", metric: "0.9950", status: "ok" });
    expect(r.runnerUps.map((x) => x.agent)).toEqual(["viola"]);
...
describe("buildHandoffKv", () => {
  it("winner branch — exact key order + winner_code_dir always emitted", () => {
    const kv = buildHandoffKv({
      ...
      winner: { agent: "violin", exp: "exp-003", approach: "augment-a2", metric: "0.9950",
                checkpoint: "workers/violin/experiments/exp-003/model.pt", notes: "best run",
                codeDir: "workers/violin/experiments/exp-003/code/" },
      runnerUps: [{ agent: "viola", exp: "exp-002", metric: "0.9100", approach: "augment-b" }],
    });
    expect(kv.split("\n").filter(Boolean)).toEqual([
      "mode=rehearsal", "topic=rehearsal-x", "landscape_doc=rehearsal-2026-05-30-x.md",
      "winner_agent=violin", "winner_exp=exp-003", "winner_approach=augment-a2", "winner_metric=0.9950",
      "winner_checkpoint=workers/violin/experiments/exp-003/model.pt", "winner_notes=best run",
      "winner_code_dir=workers/violin/experiments/exp-003/code/",
  ...
  it("winner branch omits conditional keys (no checkpoint/notes/landscape), approach default unknown", () => {
    const kv = buildHandoffKv({
      ...
      winner: { agent: "violin", exp: "exp-003", approach: "", metric: "0.99",
                codeDir: "workers/violin/experiments/exp-003/code/" },
      runnerUps: [{ agent: "viola", exp: "exp-002", metric: "0.91", approach: "" }],
    });
    expect(kv.split("\n").filter(Boolean)).toEqual([
      "mode=rehearsal", "topic=rehearsal-x",
      "winner_agent=violin", "winner_exp=exp-003", "winner_approach=unknown", "winner_metric=0.99",
      "winner_code_dir=workers/violin/experiments/exp-003/code/",
