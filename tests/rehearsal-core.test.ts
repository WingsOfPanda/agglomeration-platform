...
describe("rehearsal art-dir paths", () => {
  it("layers _rehearsal/workers/<agent>/experiments/<exp-id>", () => {
    ...
    expect(workerStateDir(art, "oboe")).toBe(`${art}/workers/oboe`);
    expect(experimentsDir(art, "oboe")).toBe(`${art}/workers/oboe/experiments`);
    expect(experimentDir(art, "oboe", "exp-001")).toBe(`${art}/workers/oboe/experiments/exp-001`);
  });
});
...
describe("buildScoreboard", () => {
  const rows: ScoreRow[] = [
    { expId: "exp-001", agent: "oboe",  metric: "0.90", status: "ok",      runtime: "10", approach: "a", metricName: "accuracy" },
    { expId: "exp-002", agent: "viola", metric: "0.95", status: "ok",      runtime: "20", approach: "b", metricName: "accuracy" },
    { expId: "exp-003", agent: "oboe",  metric: "0.95", status: "ok",      runtime: "5",  approach: "c", metricName: "accuracy" },
    { expId: "exp-004", agent: "viola", metric: "",     status: "fail",    runtime: "2",  approach: "d", metricName: "accuracy" },
    { expId: "exp-005", agent: "oboe",  metric: "",     status: "partial", runtime: "1",  approach: "e", metricName: "accuracy" },
  ];
  ...
  it("orders ok rows metric-ASC for a minimize objective (lowest = best = rank 1)", () => {
    const minRows: ScoreRow[] = [
      { expId: "exp-001", agent: "oboe",  metric: "0.50", status: "ok", runtime: "10", approach: "a", metricName: "loss" },
      { expId: "exp-002", agent: "viola", metric: "0.20", status: "ok", runtime: "20", approach: "b", metricName: "loss" },
      { expId: "exp-003", agent: "oboe",  metric: "0.35", status: "ok", runtime: "5",  approach: "c", metricName: "loss" },
    ];
  ...
  it("routes an ok+infeasible row to the xN group below the ranked rows", () => {
    const rows2: ScoreRow[] = [
      { expId: "exp-001", agent: "oboe",  metric: "0.90", status: "ok", runtime: "10", approach: "a", metricName: "accuracy" },
      { expId: "exp-002", agent: "viola", metric: "0.99", status: "ok", runtime: "20", approach: "b", metricName: "accuracy", infeasibleReason: "mismatch" },
    ];
  ...
  it("checkCompletion ignores an infeasible row (no checkCompletion change)", () => {
    const sb = buildScoreboard([
      ...
      { expId: "exp-002", agent: "oboe", metric: "0.99", status: "ok", runtime: "1", approach: "a", metricName: "accuracy", infeasibleReason: "under-run" },
    ]);
...
describe("rehearsalExperiment", () => {
  it("EXP_ID_RE / AGENT_RE match the bash regexes", () => {
    ...
    expect(AGENT_RE.test("violin")).toBe(true);
    expect(AGENT_RE.test("french-horn")).toBe(true);
    expect(AGENT_RE.test("Violin")).toBe(false);
  ...
  it("formatPeersBlock empty for zero peers, else a ## Peers table", () => {
    ...
    const peers: PeerRow[] = [{ agent: "viola", phase: "working", currentExp: "exp-003",
...
describe("rehearsalScore", () => {
  ...
  it("buildResultsTsv appends rows in given order (approach col3, metric col4)", () => {
    const tsv = buildResultsTsv([
      { expId: "exp-001", agent: "viola", approach: "base", metric: "0.9", status: "ok", runtime: "12", metricName: "accuracy" },
    ]);
  ...
  it("computeScore validates, sorts, race-guards phase clear", () => {
    const files: Record<string, string> = {
      ...
      "/a/workers/viola/state.txt": "phase=working\ncurrent_exp_id=exp-001\n",
      "/a/workers/cello/state.txt": "phase=working\ncurrent_exp_id=exp-001\n",
      "/a/workers/viola/experiments/exp-001/result.json": JSON.stringify({
      ...
      "/a/workers/cello/experiments/exp-001/result.json": JSON.stringify({
    ...
    expect(c.phaseClears.map((p) => p.statePath).sort()).toEqual([
      "/a/workers/cello/state.txt", "/a/workers/viola/state.txt"]);
  ...
  it("computeScore rejects a bad metric_name -> sidecar, omits from scoreboard+tsv, no throw", () => {
    const files: Record<string, string> = {
      "/a/metric.md": "**Primary metric:** accuracy\n",
      "/a/workers/viola/state.txt": "current_exp_id=exp-001\n",
      "/a/workers/viola/experiments/exp-001/result.json": JSON.stringify({
    ...
    expect(c.sidecars[0].path).toBe("/a/workers/viola/experiments/exp-001/result-validation.txt");
  ...
  it("computeScore removes a stale sidecar when a result becomes valid", () => {
    const files: Record<string, string> = {
      ...
      "/a/workers/viola/experiments/exp-001/result.json": JSON.stringify({
        branch_id:"b",approach_label:"x",metric_name:"accuracy",metric_value:0.9,status:"ok",
        runtime_s:1,log_paths:[],checkpoint_path:null,notes:"" }),
      "/a/workers/viola/experiments/exp-001/result-validation.txt": "FAILED at old: x\n",
    };
    const c = computeScore("/a", fakeFs(files), () => "T");
    expect(c.staleSidecars).toEqual(["/a/workers/viola/experiments/exp-001/result-validation.txt"]);
  });
  ...
  it("computeScore threads metric.md Direction: minimize -> lowest metric ranks first", () => {
    const files: Record<string, string> = {
      ...
      "/a/workers/viola/state.txt": "current_exp_id=exp-001\n",
      "/a/workers/cello/state.txt": "current_exp_id=exp-001\n",
      "/a/workers/viola/experiments/exp-001/result.json": JSON.stringify({
  ...
  it("computeScore snapshots verify-manifest.json once for a verify-bearing result", () => {
    const files: Record<string, string> = {
      ...
      "/a/workers/viola/experiments/exp-001/preds.json": "PREDS",
    };
    const c = computeScore("/a", fakeFs(files), () => "T");
    const man = c.manifests.find((m) => m.path === "/a/workers/viola/experiments/exp-001/verify-manifest.json");
  ...
  it("computeScore writes no manifest when one already exists (idempotent)", () => {
    const files: Record<string, string> = {
      ...
      "/a/workers/viola/experiments/exp-001/verify-manifest.json": "{\"command\":\"c\",\"hashes\":{}}\n",
    };
  ...
  it("computeScore emits sanity rows (ceiling) for a verify-less suspect result", () => {
    ...
    expect(ceil).toMatchObject({ expId: "exp-001", agent: "viola" });
  ...
  it("computeScore emits audit-knob-drift from prompt.md vs audit.json", () => {
    const files: Record<string, string> = {
      ...
      "/a/workers/viola/experiments/exp-001/result.json": JSON.stringify({
        ...
        runtime_s:50,log_paths:[],checkpoint_path:null,notes:"",
        integrity:{ split_before_fit:true, no_train_test_overlap:true, target_not_in_features:true, trained_steps:10, seed:1 } }),
      "/a/workers/viola/experiments/exp-001/prompt.md": "**Hard constraints:**\nmcts_sims = 200\n\n",
      "/a/workers/viola/experiments/exp-001/audit.json": JSON.stringify({ mcts_sims: 16 }),
    };
    const c = computeScore("/a", fakeFs(files), () => "T");
  ...
  it("routes a C1 not-reproduced result to the infeasible group (C1)", () => {
    const files: Record<string, string> = {
      ...
      "/a/workers/oboe/experiments/exp-001/result.json": JSON.stringify({
        branch_id:"b",approach_label:"x",metric_name:"accuracy",metric_value:0.99,status:"ok",
  ...
  it("emits per-family coverageRows over ok experiments (B1)", () => {
    const files: Record<string, string> = {
      ...
      "/a/workers/oboe/experiments/exp-001/result.json": JSON.stringify({
        ...
        runtime_s:5,log_paths:[],checkpoint_path:null,notes:"" }),
      "/a/workers/oboe/experiments/exp-002/result.json": JSON.stringify({
        branch_id:"b",approach_label:"Single-Pass",metric_name:"accuracy",metric_value:0.96,status:"ok",
        runtime_s:5,log_paths:[],checkpoint_path:null,notes:"" }),
      "/a/workers/viola/experiments/exp-003/result.json": JSON.stringify({
  ...
  it("excludes A2-infeasible (mismatch) experiments from the coverage tally (B1 x A2)", () => {
    const files: Record<string, string> = {
      ...
      "/a/workers/oboe/experiments/exp-001/result.json": JSON.stringify({
        branch_id:"b",approach_label:"single-pass",metric_name:"accuracy",metric_value:0.96,status:"ok",
        runtime_s:5,log_paths:[],checkpoint_path:null,notes:"" }),
      "/a/workers/viola/experiments/exp-002/result.json": JSON.stringify({
  ...
  it("classifies lineage from lineage.txt + parent audit diff (B2)", () => {
    ...
    const files: Record<string, string> = {
      "/a/metric.md": "**Primary metric:** accuracy\n**Direction:** maximize\n",
      "/a/workers/oboe/experiments/exp-001/result.json": okR("single-pass", 0.90),
      "/a/workers/oboe/experiments/exp-001/audit.json": JSON.stringify({ lr: 0.1, depth: 4 }),
      "/a/workers/oboe/experiments/exp-002/result.json": okR("single-pass", 0.93),
      "/a/workers/oboe/experiments/exp-002/audit.json": JSON.stringify({ lr: 0.2, depth: 4 }),
      "/a/workers/oboe/experiments/exp-002/lineage.txt": "parent_id=exp-001\n",
      "/a/workers/oboe/experiments/exp-003/result.json": okR("single-pass", 0.95),
      "/a/workers/oboe/experiments/exp-003/audit.json": JSON.stringify({ lr: 0.3, depth: 8 }),
      "/a/workers/oboe/experiments/exp-003/lineage.txt": "parent_id=exp-002\n",
    };
    const c = computeScore("/a", fakeFs(files), () => "T");
...
describe("rehearsalMonitor", () => {
  ...
  it("byte-tail emits done/error/question/heartbeat for new lines, advances offset, uses 'worker' key", () => {
    ...
    expect(r.notifications.find((n) => n.event === "done")!.worker).toBe("viola");
...
describe("rehearsalBrief", () => {
  ...
  const worker = (over: Partial<import("../src/core/rehearsalBrief.js").WorkerBrief> = {}) => ({
    agent: "viola", phase: "idle", currentOrLast: "exp-001", approach: "baseline", metric: "0.95 ok", ...over,
  });
  ...
  it("header names the just-landed exp when latest is given", () => {
    const out = buildStatusBrief({ workers: [], scoreboardMd: null, completion: SIG,
      latest: { agent: "viola", exp: "exp-003" } });
  ...
  it("tags an improve-multi top-3 leader with [multi-change] (B2)", () => {
    ...
    const out = buildStatusBrief({ workers: [], scoreboardMd: sb, completion: SIG, multiChange: { "oboe/exp-003": true } });
  ...
  it("tags top-3 rows with [reimpl-*] from inspections (C1)", () => {
    ...
    expect(buildStatusBrief({ workers: [], scoreboardMd: sb, completion: SIG, inspections: { "oboe/exp-003": "reproduced" } })).toContain("[reimpl-ok]");
    expect(buildStatusBrief({ workers: [], scoreboardMd: sb, completion: SIG, inspections: { "oboe/exp-003": "not-reproduced" } })).toContain("[reimpl-mismatch!]");
    expect(buildStatusBrief({ workers: [], scoreboardMd: sb, completion: SIG, inspections: { "oboe/exp-003": "inconclusive" } })).toContain("[reimpl-inconclusive]");
