...
function loadArgsFileVerbatim(path6, valueFlags) {
  if (!(0, import_node_fs.existsSync)(path6)) return [];
  const raw = (0, import_node_fs.readFileSync)(path6, "utf8");
  const isWs = (c3) => c3 === " " || c3 === "	" || c3 === "\n" || c3 === "\r";
  const flags = [];
  let i2 = 0;
  for (; ; ) {
    while (i2 < raw.length && isWs(raw[i2])) i2++;
    if (i2 >= raw.length) break;
    if (!(raw[i2] === "-" && raw[i2 + 1] === "-")) break;
    let j = i2;
    while (j < raw.length && !isWs(raw[j])) j++;
    const flag = raw.slice(i2, j);
    flags.push(flag);
    i2 = j;
    if (valueFlags.has(flag) && !flag.includes("=")) {
      ...
      if (k > i2) {
        flags.push(raw.slice(i2, k));
        i2 = k;
      }
    }
  }
  const body = raw.slice(i2).trim();
...
function stateArchive(agent, model, topic, suffix, opts) {
  ...
  let base = (0, import_node_path2.join)(globalRoot(), "archive", repoHash(), topic, `${agent}-${model}-${ts}`);
...
function readIfExists(path6) {
  return (0, import_node_fs5.existsSync)(path6) ? (0, import_node_fs5.readFileSync)(path6, "utf8") : "";
}
...
function outboxPath(i2, m, t) {
  return (0, import_node_path3.join)(workerDir(i2, m, t), "outbox.jsonl");
}
function identityPath(i2, m, t) {
  return (0, import_node_path3.join)(workerDir(i2, m, t), "identity.md");
}
function statusPath(i2, m, t) {
  return (0, import_node_path3.join)(workerDir(i2, m, t), "status.json");
}
function paneMetaPath(i2, m, t) {
  return (0, import_node_path3.join)(workerDir(i2, m, t), "pane.json");
}
function workerBusyState(i2, m, t) {
  const sp = statusPath(i2, m, t);
  if (!(0, import_node_fs6.existsSync)(sp)) return null;
  const match = (0, import_node_fs6.readFileSync)(sp, "utf8").match(/"state":"([^"]*)"/);
...
function identityWrite(i2, m, t) {
  ...
  const stateDir = workerDir(i2, m, t);
  const outbox = outboxPath(i2, m, t);
  let body = (0, import_node_fs6.readFileSync)(tplPath, "utf8").replaceAll("{{agent}}", i2).replaceAll("{{model}}", m).replaceAll("{{topic}}", t).replaceAll("{{state_dir}}", stateDir);
  body += `
...
\`{"event":"ready","ts":"<ISO-8601 UTC>","agent":"` + i2 + '","model":"' + m + `"}\`

Generate the timestamp at the moment you emit. Use this shell command verbatim:

\`echo "{\\"event\\":\\"ready\\",\\"ts\\":\\"$(date -u +'%Y-%m-%dT%H:%M:%SZ')\\",\\"agent\\":\\"` + i2 + '\\",\\"model\\":\\"' + m + '\\"}" >> ' + outbox + `\`
...
function readFrom(path6, offset) {
  try {
    ...
    const fd = (0, import_node_fs6.openSync)(path6, "r");
    try {
...
function resolveModel(agent, topic) {
  const td = topicDir(topic);
  if (!(0, import_node_fs6.existsSync)(td)) return null;
  const d = (0, import_node_fs6.readdirSync)(td, { withFileTypes: true }).find((e) => e.isDirectory() && e.name.startsWith(`${agent}-`));
  if (!d) return null;
  const model = d.name.slice(agent.length + 1);
  return readPaneJson(workerDir(agent, model, topic))?.model ?? model;
}
var import_node_fs6, import_node_path3, SENDER_RE, sleep;
var init_ipc = __esm({
  "src/core/ipc.ts"() {
    "use strict";
...
function quickArtDir(topic) {
  return (0, import_node_path4.join)(topicDir(topic), "_quick");
}
function quickExecDir(topic) {
  return (0, import_node_path4.join)(quickArtDir(topic), "execute");
}
...
function detectTestCommand(root) {
  if ((0, import_node_fs7.existsSync)((0, import_node_path4.join)(root, "tests", "run.sh"))) return "bash tests/run.sh";
  const pkg = (0, import_node_path4.join)(root, "package.json");
  if ((0, import_node_fs7.existsSync)(pkg)) {
    try {
      if (JSON.parse((0, import_node_fs7.readFileSync)(pkg, "utf8"))?.scripts?.test) return "npm test";
    } catch {
    }
  }
  const mk = (0, import_node_path4.join)(root, "Makefile");
  if ((0, import_node_fs7.existsSync)(mk)) {
    try {
      if (/^test:/m.test((0, import_node_fs7.readFileSync)(mk, "utf8"))) return "make test";
    } catch {
    }
  }
...
var import_node_path4, import_node_fs7;
...
function designArtDir(topic, opts) {
  return (0, import_node_path5.join)(topicDir(topic, opts), "_design");
}
function designDraftDir(topic, opts) {
  return (0, import_node_path5.join)(designArtDir(topic, opts), "design-doc", ".draft");
}
...
function designDocPath(topic, dateUtc, opts) {
  return (0, import_node_path5.join)(designArtDir(topic, opts), "design-doc", `${dateUtc}-${topic}-design.md`);
}
...
function verifyScopeFiles(target, agents) {
  const out = [];
  for (const c3 of agents) if (c3 !== target) out.push(`${c3}_only_items.txt`);
  if (agents.length >= 3) {
    for (let i2 = 0; i2 < agents.length; i2++) {
      for (let j = i2 + 1; j < agents.length; j++) {
        const a2 = agents[i2], b = agents[j];
...
function resolveDrilldownPath(scratchDir, section, agent) {
  ...
  let cand = base;
  let n2 = 2;
  while ((0, import_node_fs8.existsSync)((0, import_node_path5.join)(scratchDir, `${cand}.md`))) {
...
function designExportDocPath(repoRoot2, basename4) {
  return (0, import_node_path5.join)(repoRoot2, "docs", "ap", "specs", basename4);
}
function exportDocTo(topic, destRoot, opts) {
  const ddir = (0, import_node_path5.join)(designArtDir(topic, opts), "design-doc");
  if (!(0, import_node_fs8.existsSync)(ddir)) return null;
  const hits = (0, import_node_fs8.readdirSync)(ddir).filter((f) => f.endsWith(`-${topic}-design.md`)).sort();
  if (hits.length === 0) return null;
  const basename4 = hits[hits.length - 1];
  const dest = designExportDocPath(destRoot, basename4);
...
var import_node_path5, import_node_fs8;
...
function loadAgentPool() {
  ...
  try {
    const doc = (0, import_yaml.parse)((0, import_node_fs9.readFileSync)(p, "utf8"));
...
function agentsInDir(dir) {
  ...
  const out = [];
  for (const name of (0, import_node_fs9.readdirSync)(dir, { withFileTypes: true })) {
...
function agentsInUseGlobally() {
  ...
  if (!(0, import_node_fs9.existsSync)(repo)) return [];
  const all = [];
  for (const t of (0, import_node_fs9.readdirSync)(repo, { withFileTypes: true })) {
    if (t.isDirectory()) all.push(...agentsInDir((0, import_node_path6.join)(repo, t.name)));
  }
...
function formatCollisionError(agent, model, topic, sessionId) {
  ...
  const sidFile = (0, import_node_path6.join)(workerDir(agent, model, topic), ".session_id");
  let owner = "";
...
function load() {
  ...
  try {
    return (0, import_yaml2.parse)((0, import_node_fs10.readFileSync)(p, "utf8")) ?? {};
  } catch {
    return {};
  }
}
...
var import_node_fs10, import_node_path7, import_yaml2, CONSULT_DEFAULTS;
...
var import_node_fs11, import_node_path11, import_node_process7, normalizeCwd, getDefaultCwd, fixCwdError;
var init_cwd = __esm({
  "node_modules/execa/lib/arguments/cwd.js"() {
...
${error.message}`;
    ...
    fixCwdError = (originalMessage, cwd) => {
      ...
      let cwdStat;
      try {
        cwdStat = (0, import_node_fs11.statSync)(cwd);
      } catch (error) {
...
var import_node_fs12, import_node_tty2, handleNativeStream, handleNativeStreamSync, getTargetFd, getTargetFdNumber, handleNativeStreamAsync, getStandardStream;
var init_native = __esm({
  "node_modules/execa/lib/stdio/native.js"() {
    ...
    getTargetFd = ({ value, optionName, fdNumber, direction }) => {
      ...
      return { type: "uint8Array", value: bufferToUint8Array((0, import_node_fs12.readFileSync)(targetFdNumber)), optionName };
    };
...
var import_node_fs13, handleStdioSync, forbiddenIfSync, forbiddenNativeIfSync, throwInvalidSyncValue, addProperties, addPropertiesSync;
var init_handle_sync = __esm({
  "node_modules/execa/lib/stdio/handle-sync.js"() {
    ...
    addPropertiesSync = {
      input: {
        ...addProperties,
        fileUrl: ({ value }) => ({ contents: [bufferToUint8Array((0, import_node_fs13.readFileSync)(value))] }),
        filePath: ({ value: { file } }) => ({ contents: [bufferToUint8Array((0, import_node_fs13.readFileSync)(file))] }),
...
var import_node_fs14, transformOutputSync, transformOutputResultSync, runOutputGeneratorsSync, serializeChunks, logOutputSync, writeToFiles;
    ...
    writeToFiles = (serializedResult, stdioItems, outputFiles) => {
      for (const { path: path6, append } of stdioItems.filter(({ type }) => FILE_TYPES.has(type))) {
        ...
        if (append || outputFiles.has(pathString)) {
          (0, import_node_fs14.appendFileSync)(path6, serializedResult);
        } else {
          outputFiles.add(pathString);
          (0, import_node_fs14.writeFileSync)(path6, serializedResult);
        }
      }
    };
  }
...
var import_node_fs15, import_node_buffer3, import_node_stream3, handleStdioAsync, forbiddenIfAsync, addProperties2, addPropertiesAsync;
var init_handle_async = __esm({
  "node_modules/execa/lib/stdio/handle-async.js"() {
    ...
    addPropertiesAsync = {
      input: {
        ...addProperties2,
        fileUrl: ({ value }) => ({ stream: (0, import_node_fs15.createReadStream)(value) }),
        filePath: ({ value: { file } }) => ({ stream: (0, import_node_fs15.createReadStream)(file) }),
      ...
      output: {
        ...addProperties2,
        fileUrl: ({ value }) => ({ stream: (0, import_node_fs15.createWriteStream)(value) }),
        filePath: ({ value: { file, append } }) => ({ stream: (0, import_node_fs15.createWriteStream)(file, append ? { flags: "a" } : {}) }),
...
function wrapLaunch(launch, hasBashrc = (0, import_node_fs16.existsSync)((0, import_node_path13.join)((0, import_node_os5.homedir)(), ".bashrc"))) {
...
async function paneOption(pane, opt) {
  try {
    return (await execa("tmux", ["display-message", "-p", "-t", pane, opt])).stdout;
  } catch {
    return "";
  }
}
async function killGraceful(pane, pluginRoot2) {
  ...
  const snap = (0, import_node_path13.join)((0, import_node_fs16.mkdtempSync)((0, import_node_path13.join)((0, import_node_os5.tmpdir)(), "cs-snap-")), "snap.txt");
  try {
    const { stdout } = await execa("tmux", ["capture-pane", "-p", "-e", "-t", pane]);
    (0, import_node_fs16.writeFileSync)(snap, stdout);
  } catch {
    (0, import_node_fs16.writeFileSync)(snap, "");
  }
...
var import_node_os5, import_node_fs16, import_node_path13, splitRight, splitDown, respawn;
var init_tmux = __esm({
  "src/core/tmux.ts"() {
    "use strict";
    init_execa();
...
function classifyStale(state, outbox, thresholdS = 180) {
  ...
  const ageS = (Date.now() - (0, import_node_fs20.statSync)(outbox).mtimeMs) / 1e3;
...
`);
  ...
  for (const t of (0, import_node_fs20.readdirSync)(repo, { withFileTypes: true })) {
    ...
    for (const p of (0, import_node_fs20.readdirSync)(td, { withFileTypes: true })) {
...
var import_node_fs20, import_node_path16, staleThresholdS;
var init_list = __esm({
  "src/commands/list.ts"() {
    "use strict";
...
function liveDeps() {
  return {
    ...
    removeLastPane: (t) => {
      try {
        (0, import_node_fs21.rmSync)((0, import_node_path17.join)(topicDir(t), ".last_pane"), { force: true });
      } catch {
      }
    }
  };
...
function collectTopicPairs(topic) {
  const td = topicDir(topic);
  if (!(0, import_node_fs21.existsSync)(td)) return [];
  const pairs = [];
  for (const name of (0, import_node_fs21.readdirSync)(td, { withFileTypes: true })) {
    if (!name.isDirectory() || isArtifactDir(name.name)) continue;
...
function collectAgentPairs(topic, agents) {
  const td = topicDir(topic);
  if (!(0, import_node_fs21.existsSync)(td)) return [];
  const dirs = (0, import_node_fs21.readdirSync)(td, { withFileTypes: true }).filter((e) => e.isDirectory());
  const pairs = [];
...
function cleanupTopicDir(topic) {
  const td = topicDir(topic);
  try {
    (0, import_node_fs21.rmSync)((0, import_node_path17.join)(td, ".last_pane"), { force: true });
  } catch {
  }
  try {
    (0, import_node_fs21.rmdirSync)(td);
  } catch {
  }
}
async function run5(args) {
  ...
  if (a0 === "--all") {
    ...
    const repo = repoStateDir();
    if (!(0, import_node_fs21.existsSync)(repo)) {
...
var import_node_fs21, import_node_path17, GRACEFUL_BATCH_WAIT_MS, sleep3;
var init_stop = __esm({
  "src/commands/stop.ts"() {
    "use strict";
...
function readProviderList(path6) {
  ...
  try {
    return parseProviderList((0, import_node_fs22.readFileSync)(path6, "utf8"));
  } catch {
    return [];
  }
}
...
function opencodeConfigPath(cwd = process.cwd(), home = (0, import_node_os6.homedir)()) {
  ...
  if ((0, import_node_fs23.existsSync)(proj)) return proj;
  const glob = (0, import_node_path18.join)(home, ".config", "opencode", "opencode.json");
  return (0, import_node_fs23.existsSync)(glob) ? glob : null;
}
function opencodePermissionCheck(cfgPath) {
  const p = cfgPath ?? opencodeConfigPath();
  if (!p || !(0, import_node_fs23.existsSync)(p)) return { rc: 1, message: "no opencode.json found" };
  let obj;
  try {
    obj = JSON.parse((0, import_node_fs23.readFileSync)(p, "utf8"));
  } catch {
...
function listSet(providers) {
  ...
  (0, import_node_fs23.mkdirSync)(root, { recursive: true });
...
function healthCheck() {
  ...
  if ((0, import_node_fs23.existsSync)(idTpl)) log.ok("config: identity.md (template present)");
  else {
...
var import_node_fs23, import_node_child_process8, import_node_path18, import_node_os6, availablePath, activePath;
...
async function run7(args) {
  ...
  if (list.length !== n2) {
    log.error(`list has ${list.length} entries, expected ${n2}`);
    return 1;
  }
  const art = artDir || (0, import_node_path19.join)(topicDir(topic), "_consult");
  (0, import_node_fs24.mkdirSync)(art, { recursive: true });
  ...
  try {
    ...
    for (const o2 of out) process.stdout.write(`  ${o2.agent}	${o2.pane}
`);
    return 0;
  } catch (e) {
...
var import_node_fs24, import_node_path19, SLUG2;
...
async function turnSendWith(topic, round, d) {
  ...
  if (!(0, import_node_fs25.existsSync)(outbox)) {
    log.error(`quick turn-send: outbox not found at ${outbox} \u2014 was ${agent} spawned?`);
    return 1;
  }
  const busy = workerBusyState(agent, provider, topic);
  if (busy) {
    log.error(`quick turn-send: worker not idle (state=${busy}); previous turn still in flight`);
    return 1;
  }
...
`);
  ...
  if (rc !== 0) {
    log.error(`quick turn-send: send failed (rc=${rc}); ${stateFile} kept for retry`);
    return 1;
  }
...
async function turnWaitWith(topic, round, d) {
  ...
  if (ts === "question" && ev) {
    atomicWrite((0, import_node_path20.join)(exec, `question-${round}.txt`), JSON.stringify(ev) + "\n");
...
var init_quick2 = __esm({
  "src/commands/quick.ts"() {
    ...
    QUICK_TURN_TIMEOUT = Number(process.env.AP_QUICK_TURN_TIMEOUT) || 14400;
  }
});
...
function adjudicateNge3(input) {
  ...
  const processBucket = (content, ownersCsv) => {
    ...
    const verifiers = agents.filter((c3) => !own.includes(c3));
  ...
  for (let i2 = 0; i2 < n2; i2++) for (let j = i2 + 1; j < n2; j++) processBucket(input.buckets[`${agents[i2]}+${agents[j]}_only.txt`], `${agents[i2]}+${agents[j]}`);
  for (const c3 of agents) processBucket(input.buckets[`${c3}_only_items.txt`], c3);
  return emitSections([
...
${(0, import_node_fs26.readFileSync)(hintFile, "utf8")}`;
}
var import_node_fs26, import_node_path21, BRAINSTORMING, DEBUGGING;
...
function walkSectionState(dir, opts) {
  let files;
  try {
    files = (0, import_node_fs27.readdirSync)(dir).filter((f) => f.endsWith(".md"));
  } catch {
    return [];
  }
  ...
  return names.map((name) => {
    const body = (0, import_node_fs27.readFileSync)((0, import_node_path22.join)(dir, `${name}.md`), "utf8").replace(/\s/g, "");
    return { name, status: body === "_(skipped)_" ? "skipped" : "approved" };
  });
}
var import_node_fs27, import_node_path22;
...
async function initWith2(tokens, d) {
  ...
  const rows = list.map((provider, i2) => ({ provider, agent: agents[i2] }));
  (0, import_node_fs28.mkdirSync)(designDraftDir(topic), { recursive: true });
  ...
  atomicWrite((0, import_node_path23.join)(art, "list.txt"), formatListFile(rows, isoUtc()));
...
async function assembleRun(rest) {
  ...
  if (!(0, import_node_fs28.existsSync)(draftDir)) {
  ...
  (0, import_node_fs28.mkdirSync)((0, import_node_path23.join)(art, "design-doc"), { recursive: true });
  atomicWrite(out, doc);
  const result = auditDoc(doc);
  const auditText = [`VERDICT=${result.verdict}`, ...result.issues.map((i2) => `ISSUE=${i2}`)].join("\n") + "\n";
  atomicWrite((0, import_node_path23.join)(art, "design-doc", "audit.log"), auditText);
...
async function researchSendRun(rest) {
  ...
  if (!topic || !agent || !provider) {
    log.error("usage: design research-send <topic> <agent> <provider>");
    return 2;
  }
...
async function researchSendWith(topic, agent, provider, d) {
  const art = designArtDir(topic);
  const stateFile = (0, import_node_path23.join)(art, `research-${agent}.txt`);
  if ((0, import_node_fs28.existsSync)(stateFile)) {
    log.error(`design research-send: ${stateFile} exists; rm to retry`);
    return 1;
  }
  const topicText = readIfExists((0, import_node_path23.join)(art, "topic.txt")).trim();
  if (!topicText) {
    log.error(`design research-send: topic.txt missing/empty at ${art} (run design init)`);
    return 1;
  }
  const findingsPath = (0, import_node_path23.join)(workerDir(agent, provider, topic), "findings.md");
  const promptFile = (0, import_node_path23.join)(art, `${agent}_research_prompt.md`);
  atomicWrite(promptFile, skillHintAppend((0, import_node_path23.join)(art, "skill.txt"), composeResearchPrompt(topicText, findingsPath)));
  const offset = d.offsetFor(agent, provider, topic);
  atomicWrite(stateFile, `OFFSET=${offset}
`);
  const rc = await d.send(["--from", "hub", agent, topic, `@${promptFile}`]);
  if (rc !== 0) {
    log.error(`design research-send: send failed (rc=${rc}); ${stateFile} kept (rm to redo)`);
    return 1;
  }
...
async function researchWaitRun(rest) {
  const [topic, agent, provider] = rest;
  if (!topic || !agent || !provider) {
    log.error("usage: design research-wait <topic> <agent> <provider>");
    return 2;
  }
  return researchWaitWith(topic, agent, provider, liveResearchWaitDeps);
}
function recordWaitOutcome(art, agent, provider, topic, stateFile, ev, state, key, phase) {
  if (state === "question" && ev) {
    atomicWrite((0, import_node_path23.join)(art, `question-${agent}.txt`), JSON.stringify(ev) + "\n");
    const bumped = outboxOffset(outboxPath(agent, provider, topic));
    (0, import_node_fs28.appendFileSync)(stateFile, `OFFSET=${bumped}
${key}=question
`);
  } else {
    (0, import_node_fs28.appendFileSync)(stateFile, `${key}=${state}
`);
  }
  (0, import_node_fs28.writeFileSync)((0, import_node_path23.join)(art, `${phase}-${agent}.done`), "");
...
async function researchWaitWith(topic, agent, provider, d) {
  ...
  const stateFile = (0, import_node_path23.join)(art, `research-${agent}.txt`);
  if (!(0, import_node_fs28.existsSync)(stateFile)) {
    log.error(`design research-wait: ${stateFile} missing (run design research-send first)`);
    return 1;
  }
  const offset = parseLatestOffset((0, import_node_fs28.readFileSync)(stateFile, "utf8"));
  if (offset === null) {
    log.error(`design research-wait: OFFSET not set in ${stateFile}`);
    return 1;
  }
...
async function diffRun(rest) {
  ...
  if ((0, import_node_fs28.existsSync)((0, import_node_path23.join)(art, "diff.md"))) {
  ...
  const workers = [];
  for (const r of rows) {
    const f = (0, import_node_path23.join)(workerDir(r.agent, r.provider, topic), "findings.md");
    if (!(0, import_node_fs28.existsSync)(f)) {
      log.error(`design diff: ${r.agent} findings.md missing: ${f}`);
      return 1;
    }
    workers.push({ name: r.agent, findings: (0, import_node_fs28.readFileSync)(f, "utf8") });
  }
  const result = diffFindings(workers);
  for (const file of result.files) atomicWrite((0, import_node_path23.join)(art, file.filename), file.content);
  atomicWrite((0, import_node_path23.join)(art, "diff.md"), result.diffMd);
  ...
  log.ok(`design diff: wrote ${(0, import_node_path23.join)(art, "diff.md")} (${rows.length} workers) ${summary}`);
  return 0;
}
async function verifySendRun(rest) {
  const [topic, agent, provider] = rest;
  if (!topic || !agent || !provider) {
    log.error("usage: design verify-send <topic> <agent> <provider>");
    return 2;
  }
...
async function verifySendWith(topic, agent, provider, d) {
  ...
  const stateFile = (0, import_node_path23.join)(art, `verify-${agent}.txt`);
  if ((0, import_node_fs28.existsSync)(stateFile)) {
  ...
  const agents = parseListFile((0, import_node_fs28.readFileSync)(listPath, "utf8")).map((r) => r.agent);
  if (agents.length < 2) {
  ...
  for (const f of verifyScopeFiles(agent, agents)) {
    const p = (0, import_node_path23.join)(art, f);
    if (!(0, import_node_fs28.existsSync)(p)) {
      log.error(`design verify-send: expected bucket missing: ${p} (run design diff first)`);
      return 1;
    }
    const c3 = (0, import_node_fs28.readFileSync)(p, "utf8");
    if (c3.split("\n").some((l) => l.length > 0)) workers.push(c3.replace(/\n+$/, ""));
  }
  const items = workers.join("\n");
  atomicWrite((0, import_node_path23.join)(art, `verify-claims-${agent}.txt`), items ? items + "\n" : "");
  if (!items) {
    atomicWrite(stateFile, "VS=skipped\n");
    log.ok(`design verify-send: ${agent} VS=skipped (no claims to verify)`);
    return 0;
  }
  const verifyPath = (0, import_node_path23.join)(workerDir(agent, provider, topic), "verify.md");
  const promptFile = (0, import_node_path23.join)(art, `${agent}_verify_prompt.md`);
  atomicWrite(promptFile, skillHintAppend((0, import_node_path23.join)(art, "skill.txt"), composeVerifyPrompt(items, verifyPath)));
  const offset = d.offsetFor(agent, provider, topic);
  atomicWrite(stateFile, `OFFSET=${offset}
`);
  const rc = await d.send(["--from", "hub", agent, topic, `@${promptFile}`]);
  if (rc !== 0) {
    log.error(`design verify-send: send failed (rc=${rc}); ${stateFile} kept (rm to redo)`);
    return 1;
  }
...
async function verifyWaitRun(rest) {
  const [topic, agent, provider] = rest;
  if (!topic || !agent || !provider) {
    log.error("usage: design verify-wait <topic> <agent> <provider>");
    return 2;
  }
...
async function verifyWaitWith(topic, agent, provider, d) {
  ...
  const stateFile = (0, import_node_path23.join)(art, `verify-${agent}.txt`);
  if (!(0, import_node_fs28.existsSync)(stateFile)) {
    log.error(`design verify-wait: ${stateFile} missing (run design verify-send first)`);
    return 1;
  }
  const text = (0, import_node_fs28.readFileSync)(stateFile, "utf8");
  if (lastTag(text, "VS") === "skipped") {
    (0, import_node_fs28.writeFileSync)((0, import_node_path23.join)(art, `verify-${agent}.done`), "");
    log.ok(`design verify-wait: ${agent} VS=skipped (already)`);
    return 0;
  }
  const offset = parseLatestOffset(text);
  if (offset === null) {
    log.error(`design verify-wait: OFFSET not set in ${stateFile}`);
    return 1;
  }
...
async function adjudicateRun(rest) {
  ...
  const vs = {};
  for (const r of rows) {
    verify[r.agent] = readIfExists((0, import_node_path23.join)(workerDir(r.agent, r.provider, topic), "verify.md"));
    vs[r.agent] = lastTag(readIfExists((0, import_node_path23.join)(art, `verify-${r.agent}.txt`)), "VS") ?? "skipped";
  }
  const buckets = {};
  const addBucket = (f) => {
    buckets[f] = readIfExists((0, import_node_path23.join)(art, f));
  };
  for (const c3 of agents) addBucket(`${c3}_only_items.txt`);
  if (agents.length >= 3) {
    addBucket("consensus.txt");
    for (let i2 = 0; i2 < agents.length; i2++) for (let j = i2 + 1; j < agents.length; j++) addBucket(`${agents[i2]}+${agents[j]}_only.txt`);
  }
  const input = { workers: rows.map((r) => ({ agent: r.agent, provider: r.provider })), verify, vs, buckets };
  atomicWrite((0, import_node_path23.join)(art, "adjudicated-draft.md"), adjudicate(input));
  log.ok(`design adjudicate: wrote ${(0, import_node_path23.join)(art, "adjudicated-draft.md")}`);
...
async function synthesizeRun(rest) {
  ...
  const art = designArtDir(topic);
  const adj = (0, import_node_path23.join)(art, "adjudicated.md");
  if (!(0, import_node_fs28.existsSync)(adj)) {
  ...
  const draftDir = designDraftDir(topic);
  (0, import_node_fs28.mkdirSync)(draftDir, { recursive: true });
  const seeds = synthesizeSeeds(adjText);
  for (const s of seeds) atomicWrite((0, import_node_path23.join)(draftDir, `${s.section}.md`), s.body);
...
async function waitGateRun(rest) {
  ...
  const listPath = (0, import_node_path23.join)(art, "list.txt");
  if (!(0, import_node_fs28.existsSync)(listPath)) {
    log.error(`design wait-gate: list.txt missing at ${art}`);
    return 2;
  }
  const rows = parseListFile((0, import_node_fs28.readFileSync)(listPath, "utf8"));
  if (rows.length === 0) {
    log.error("design wait-gate: list.txt has no workers");
    return 2;
  }
  const key = phase === "research" ? "FS" : "VS";
  const workers = rows.map((r) => {
    const stateFile = (0, import_node_path23.join)(art, `${phase}-${r.agent}.txt`);
    return {
      agent: r.agent,
      doneExists: (0, import_node_fs28.existsSync)((0, import_node_path23.join)(art, `${phase}-${r.agent}.done`)),
...
async function drilldownWith(rest, d, hooks) {
  ...
  const [i2, m2] = n2 >= 9 ? [rest[7], rest[8]] : ["", ""];
  if (!(0, import_node_fs28.existsSync)(ddDir)) {
    log.error(`design drilldown: dd-dir not found: ${ddDir}`);
    return 2;
  }
  if (!(0, import_node_fs28.existsSync)(designDoc)) {
    log.error(`design drilldown: design-doc not found: ${designDoc}`);
    return 2;
  }
  const scratch = (0, import_node_path23.join)(ddDir, "_scratch");
  (0, import_node_fs28.mkdirSync)(scratch, { recursive: true });
  ...
  const results = await Promise.all(jobs.map(async (j) => {
    const promptFile = (0, import_node_path23.join)(scratch, `.${j.inst}-drill-prompt.md`);
...
async function offsetResetRun(rest) {
  ...
  for (const f of [`${phase}-${agent}.txt`, `${phase}-${agent}.done`, `question-${agent}.txt`])
    (0, import_node_fs28.rmSync)((0, import_node_path23.join)(art, f), { force: true });
  const c3 = cascadeTargets(phase, keepFindings);
  if (!keepFindings) {
    const td = topicDir(topic);
    if ((0, import_node_fs28.existsSync)(td)) {
      for (const name of (0, import_node_fs28.readdirSync)(td))
        if (name.startsWith(`${agent}-`)) (0, import_node_fs28.rmSync)((0, import_node_path23.join)(td, name, c3.workerFile), { force: true });
    }
    for (const f of c3.artFiles) (0, import_node_fs28.rmSync)((0, import_node_path23.join)(art, f), { force: true });
    const names = (0, import_node_fs28.readdirSync)(art);
    for (const g of c3.artGlobs) {
      const re = new RegExp("^" + g.replace(/[.]/g, "\\.").replace(/\*/g, ".*") + "$");
      for (const n2 of names) if (re.test(n2)) (0, import_node_fs28.rmSync)((0, import_node_path23.join)(art, n2), { force: true });
    }
  }
...
var import_node_fs28, import_node_path23, liveInitDeps2, liveSpawnAllDeps, liveResearchSendDeps, liveResearchWaitDeps, DRILLDOWN_TIMEOUT;
...
function implementArtDir(topic, opts) {
  ...
  return (0, import_node_path24.join)(topicDir(topic, opts), "_implement");
}
...
var import_node_path24, import_node_fs29, ImplementArgError;
...
var import_node_path25, BRANCH_DISCIPLINE2;
...
async function findLatestDocRun(rest) {
  ...
  let best = null;
  if ((0, import_node_fs30.existsSync)(stateDir)) for (const topic of (0, import_node_fs30.readdirSync)(stateDir)) {
    const dd = (0, import_node_path26.join)(stateDir, topic, "_design", "design-doc");
    if (!(0, import_node_fs30.existsSync)(dd)) continue;
    for (const f of (0, import_node_fs30.readdirSync)(dd)) {
      if (!f.endsWith("-design.md")) continue;
      const p = (0, import_node_path26.join)(dd, f);
      let mt = 0;
      try {
        mt = (0, import_node_fs30.statSync)(p).mtimeMs;
      } catch {
        continue;
      }
...
async function auditRun(rest) {
  ...
  if (!(0, import_node_fs30.existsSync)(doc)) {
    log.error(`implement audit: doc unreadable: ${doc}`);
    return 2;
  }
  let text;
  try {
    text = (0, import_node_fs30.readFileSync)(doc, "utf8");
  } catch {
    log.error(`implement audit: doc unreadable: ${doc}`);
    return 2;
  }
...
async function turnSendWith2(topic, round, d) {
  ...
  if (!(0, import_node_fs30.existsSync)(outbox)) {
    log.error(`implement turn-send: outbox not found at ${outbox} \u2014 was ${WORKER} spawned?`);
    return 1;
  }
  const busy = workerBusyState(WORKER, model, topic);
  if (busy) {
    log.error(`implement turn-send: worker not idle (state=${busy}); previous turn still in flight`);
    return 1;
  }
...
async function branchWith2(a2, opts, runnerFor) {
  ...
  atomicWrite((0, import_node_path26.join)(art, "implement-branches.tsv"), rows.length ? rows.join("\n") + "\n" : "");
...
async function scopeCheckWith(topic, d) {
  const art = implementArtDir(topic);
  const designFile = (0, import_node_path26.join)(art, "design.md");
  const targetFile = (0, import_node_path26.join)(art, "target_cwd.txt"), baseFile = (0, import_node_path26.join)(art, "branch-base.sha");
...
async function summaryWith(topic, d) {
  ...
  for (const t of iterTargets(topic)) {
    ...
    if (!(0, import_node_fs30.existsSync)(baseline)) {
      log.error(`implement summary: baseline missing for slug=${t.slug} (${baseline})`);
      continue;
    }
    if (!isDir(t.cwd)) {
      log.warn(`implement summary: target gone for slug=${t.slug} (cwd=${t.cwd}); omitting block`);
      continue;
    }
...
function applyFinish(art, t, action, d) {
  const branch = branchMapField((0, import_node_path26.join)(art, "implement-branches.tsv"), t.slug);
  const startBranch = kvField((0, import_node_path26.join)(art, "baselines", `${t.slug}.tsv`), "branch");
...
function forensicsRoot() {
  return (0, import_node_path27.join)(globalRoot(), "forensics");
}
function walkForensics(root, includeReviewed) {
  ...
  const walk = (dir) => {
    let entries;
    try {
      entries = (0, import_node_fs31.readdirSync)(dir, { withFileTypes: true });
    } catch {
      return;
    }
    for (const e of entries) {
      const p = (0, import_node_path27.join)(dir, e.name);
      if (e.isDirectory()) {
  ...
  if ((0, import_node_fs31.existsSync)(root)) walk(root);
  return out.sort();
}
function readLedgerText(root) {
  try {
    return (0, import_node_fs31.readFileSync)((0, import_node_path27.join)(root, ".trends.json"), "utf8");
  } catch {
    return null;
  }
}
async function surveyWith(o2) {
  ...
  for (const f of files) {
    let text;
    try {
      text = (0, import_node_fs31.readFileSync)(f, "utf8");
    } catch {
      continue;
    }
    ...
    if (cutoff !== null) {
      let mt = 0;
      try {
        mt = (0, import_node_fs31.statSync)(f).mtimeMs;
      } catch {
      }
...
`);
  ...
  log.info(`review survey: ${n2} forensics file(s)`);
  return 0;
}
async function archiveWith(paths, o2 = {}) {
  ...
  for (const p of paths) {
    ...
    let text;
    try {
      text = (0, import_node_fs31.readFileSync)(p, "utf8");
    } catch {
    ...
    try {
      (0, import_node_fs31.mkdirSync)((0, import_node_path27.dirname)(target), { recursive: true });
      (0, import_node_fs31.renameSync)(p, target);
    } catch (e) {
  ...
  atomicWrite((0, import_node_path27.join)(root, ".trends.json"), JSON.stringify(ledger, null, 2) + "\n");
  ...
  if (verb === "survey") {
    ...
    for (let i2 = 0; i2 < rest.length; i2++) {
      if (rest[i2] === "--all") o2.all = true;
        ...
        log.error(`review survey: unknown flag '${rest[i2]}'`);
        return 2;
      }
    }
...
function workersDir(artDir) {
  return (0, import_node_path28.join)(artDir, "workers");
}
function workerStateDir(artDir, agent) {
  return (0, import_node_path28.join)(workersDir(artDir), agent);
}
function experimentsDir(artDir, agent) {
  return (0, import_node_path28.join)(workerStateDir(artDir, agent), "experiments");
}
function experimentDir(artDir, agent, expId) {
  return (0, import_node_path28.join)(experimentsDir(artDir, agent), expId);
}
function seedLib(art, configRoot) {
  try {
    const seedDir = (0, import_node_path28.join)(configRoot, "config", "autoresearch-lib-seed");
    if (!(0, import_node_fs32.existsSync)(seedDir)) return;
    const dest = (0, import_node_path28.join)(art, "lib");
    (0, import_node_fs32.mkdirSync)(dest, { recursive: true });
    for (const name of (0, import_node_fs32.readdirSync)(seedDir)) {
      const src = (0, import_node_path28.join)(seedDir, name);
      if (!(0, import_node_fs32.statSync)(src).isFile()) continue;
      const target = (0, import_node_path28.join)(dest, name);
      if (!(0, import_node_fs32.existsSync)(target)) (0, import_node_fs32.copyFileSync)(src, target);
    }
  } catch {
  }
}
...
function knobsDiffer(a2, b) {
  const x = parseFloat(String(a2)), y = parseFloat(String(b));
  return !Number.isNaN(x) && !Number.isNaN(y) ? x !== y : String(a2) !== String(b);
}
function diffAuditKnobs(parentAudit, childAudit) {
  ...
  let n2 = 0;
  for (const k of keys) {
    if (knobsDiffer(parentAudit[k], childAudit[k])) n2 += 1;
  }
  return n2;
}
...
function computeScore(art, fs, now) {
  const metricMd = fs.read((0, import_node_path29.join)(art, "metric.md"));
  const parsed = metricMd ? parseMetricMd(metricMd) : null;
  const verdicts = parseVerdicts2(fs.read((0, import_node_path29.join)(art, "verification.tsv")) ?? "");
  const inspections = parseInspections(fs.read((0, import_node_path29.join)(art, "inspection.tsv")) ?? "");
  ...
  for (const agent of workers) {
    ...
    for (const expId of exps) {
      ...
      const resultPath = (0, import_node_path29.join)(branchDir, "result.json");
      if (!fs.exists(resultPath)) continue;
      const sidecar = (0, import_node_path29.join)(branchDir, "result-validation.txt");
      let json;
      try {
      ...
      const v = validateResult(json, {
        expectedMetric,
        logPathExists: (p) => p.startsWith("./") ? fs.exists((0, import_node_path29.join)(branchDir, p)) : true
      });
      if (!v.ok) {
...
` });
      ...
      if (vblock && vblock.kind !== "none" && vblock.command) {
        const manifestPath = (0, import_node_path29.join)(branchDir, "verify-manifest.json");
        if (!fs.exists(manifestPath)) {
          const manifest = buildManifest(vblock, (rel) => fs.read((0, import_node_path29.join)(branchDir, rel)));
          if (manifest) manifests.push({ path: manifestPath, body: JSON.stringify(manifest) + "\n" });
        }
      }
      const promptMd = fs.read((0, import_node_path29.join)(branchDir, "prompt.md"));
      const auditObj = parseAudit(fs.read((0, import_node_path29.join)(branchDir, "audit.json")));
      const flags = sanityFlags({
        result: o2,
        direction: parsed?.direction,
        ceiling: parsed?.ceiling,
        minRuntimeS: parsed?.minRuntimeS ?? 1,
        readLog: (rel) => fs.read((0, import_node_path29.join)(branchDir, rel)),
      ...
      const lineageTxt = fs.read((0, import_node_path29.join)(branchDir, "lineage.txt"));
      const parentId = lineageTxt ? parseState(lineageTxt).parent_id ?? "" : "";
      let knobs = null;
      if (parentId) {
        const parentAudit = parseAudit(fs.read((0, import_node_path29.join)(experimentDir(art, agent, parentId), "audit.json")));
  ...
  const phaseClears = [];
  for (const agent of workers) {
    const statePath = (0, import_node_path29.join)(workerStateDir(art, agent), "state.txt");
    const stateTxt = fs.read(statePath);
    if (stateTxt === null) continue;
    const cur = parseState(stateTxt).current_exp_id ?? "";
    if (!cur) continue;
    if (!fs.exists((0, import_node_path29.join)(experimentDir(art, agent, cur), "result.json"))) continue;
...
var import_node_path29, TSV_HEADER;
...
function parseRows(scoreboardMd) {
  ...
  for (const line of scoreboardMd.split("\n")) {
    ...
    out.push({ exp: c3[2], agent: c3[3], metric: c3[4], status: c3[5], metricName: c3[8] ?? "", approach: c3[7] ?? "" });
  }
  return out;
}
function checkCompletion(scoreboardMd, metricMd) {
  ...
  const tuples = [...allRows].sort((a2, b) => (a2.agent < b.agent ? -1 : a2.agent > b.agent ? 1 : 0) || (a2.exp < b.exp ? -1 : a2.exp > b.exp ? 1 : 0));
...
function parseTopRows(scoreboardMd) {
  ...
  for (const line of scoreboardMd.split("\n")) {
    if (!/^\|\s+\d+\s+\|\s+exp-/.test(line)) continue;
    const c3 = line.split("|").map((s) => s.trim());
    out.push({ rank: c3[1], exp: c3[2], agent: c3[3], metric: c3[4], metricName: c3[8] ?? "" });
  }
  return out;
}
function yn(b) {
...
function parseScoreboard(md) {
  ...
  for (const line of md.split("\n")) {
    ...
    const c3 = line.split("|").map((s) => s.trim());
    rows.push({ rank: c3[1], expId: c3[2], agent: c3[3], metric: c3[4], status: c3[5] });
  }
...
function buildHandoffKv(i2) {
  const L = [];
  if (!i2.winner) {
    L.push("mode=autoresearch-no-winner", `topic=${i2.topic}`);
  ...
  const w = i2.winner;
  L.push("mode=autoresearch", `topic=${i2.topic}`);
  if (i2.landscapeDoc) L.push(`landscape_doc=${i2.landscapeDoc}`);
  L.push(
...
function fingerprint(d) {
  ...
  let h2 = 5381;
  for (const c3 of basis) h2 = (h2 << 5) + h2 + c3.charCodeAt(0) >>> 0;
  return "l" + h2.toString(16);
}
...
function lessonsPath(storeRoot, repoHash2, metricFamily) {
  return (0, import_node_path30.join)(storeRoot, scopeKey(repoHash2, metricFamily), "lessons.jsonl");
}
function readLessons(io, path6) {
  if (!io.exists(path6)) return [];
  let text;
  try {
    text = io.readFile(path6);
  } catch {
    return [];
  }
  const out = [];
...
function writeLessonsAtFinalize(io, opts) {
  ...
  store.forEach((l, i2) => byId.set(l.id, i2));
  let mutated = false;
  for (let i2 = 0; i2 < drafts.length; i2++) {
    const draft = drafts[i2];
    const verdict = verdicts[i2];
...
var init_autoresearchMemoryStore = __esm({
  "src/core/autoresearchMemoryStore.ts"() {
    ...
    liveMemoryIo = {
      ...
      readFile: (p) => (0, import_node_fs33.readFileSync)(p, "utf8"),
      mkdir: (p) => (0, import_node_fs33.mkdirSync)(p, { recursive: true }),
...
async function initWith4(args, deps) {
  ...
  if ((0, import_node_fs34.existsSync)(art)) {
    log.error(`autoresearch init: topic already in flight: ${art}`);
    return 2;
  }
  if (p.seedFrom && !(0, import_node_fs34.existsSync)(p.seedFrom)) {
    log.error(`autoresearch init: --seed-from not found: ${p.seedFrom}`);
    return 1;
  }
  (0, import_node_fs34.mkdirSync)(art, { recursive: true });
  ...
  if (p.metric !== void 0) {
    ...
    atomicWrite((0, import_node_path31.join)(art, "metric.md"), formatMetricBlock(frameMetric(p.topic)));
  }
  ...
  if (autonomous) atomicWrite((0, import_node_path31.join)(art, "autonomous.txt"), "1\n");
  out(`TOPIC=${slug}`);
  out(`ART=${art}`);
  return 0;
}
...
async function metricWith(args, v = {}) {
  ...
  try {
    atomicWrite((0, import_node_path31.join)(autoresearchArtDir(topic, v.opts), "metric.md"), formatMetricBlock(parseKv(kv)));
  } catch (e) {
...
async function sotaWith(args, v = {}) {
  ...
  try {
    atomicWrite(
      (0, import_node_path31.join)(autoresearchArtDir(topic, v.opts), "sota.md"),
...
async function spawnAllWith2(args, deps, opts) {
  ...
  const staleResults = (0, import_node_path31.join)(art, "spawn-results.tsv");
  if ((0, import_node_fs34.existsSync)(staleResults)) (0, import_node_fs34.rmSync)(staleResults);
  const agents = deps.pickAgents(topic, n2);
  if (agents.length < 2) {
  ...
  atomicWrite((0, import_node_path31.join)(art, "workers.txt"), agents.join("\n") + "\n");
  ...
  const panes = parsePanesFile((0, import_node_fs34.readFileSync)((0, import_node_path31.join)(art, "preflight-panes.txt"), "utf8"));
  const orphans = rows.filter((r) => !panes.has(r.agent));
  if (orphans.length) {
...
async function dropWorkerWith(rest, deps, opts) {
  ...
  const art = autoresearchArtDir(topic, opts);
  const workersFile = (0, import_node_path31.join)(art, "workers.txt");
  if (!(0, import_node_fs34.existsSync)(workersFile)) {
    log.error(`autoresearch drop-worker: workers.txt missing`);
    return 1;
  }
  const kept = [];
  let dropped = false;
  for (const line of (0, import_node_fs34.readFileSync)(workersFile, "utf8").split("\n")) {
  ...
  const panesFile = (0, import_node_path31.join)(art, "preflight-panes.txt");
  if ((0, import_node_fs34.existsSync)(panesFile)) {
    try {
      const pane = parsePanesFile((0, import_node_fs34.readFileSync)(panesFile, "utf8")).get(agent);
...
async function verifyCheckWith(args, deps) {
  ...
  if (!runFailed) {
    ...
    recomputed = stdout === null ? null : recomputedFromOutput(stdout, metricFrom, (p) => deps.readJson((0, import_node_path31.join)(experimentDir(art, agent, expId), p)));
  }
...
async function inspectPlanWith(args, deps) {
  ...
  out(`INSPECT_CWD=${(0, import_node_path31.join)(experimentDir(art, agent, expId), "c1")}`);
...
async function inspectCheckWith(args, deps) {
  ...
  if (!runFailed && !integrityRefuted) {
    const stdout = stdoutFile ? deps.readStdout(stdoutFile) : null;
    reimplMetric = stdout === null ? null : recomputedFromOutput(stdout, "marker", (p) => deps.readJson((0, import_node_path31.join)(experimentDir(art, agent, expId), p)));
  }
...
function gatherPeers(art, self) {
  const workersFile = (0, import_node_path31.join)(art, "workers.txt");
  if (!(0, import_node_fs34.existsSync)(workersFile)) return [];
  const peers = (0, import_node_fs34.readFileSync)(workersFile, "utf8").split("\n").map((l) => l.trim()).filter((l) => l && l !== self);
  const rows = [];
  for (const peer of peers) {
    const peerDir = workerStateDir(art, peer);
    if (!(0, import_node_fs34.existsSync)(peerDir)) continue;
    let phase = "", currentExp = "";
    const statePath = (0, import_node_path31.join)(peerDir, "state.txt");
    if ((0, import_node_fs34.existsSync)(statePath)) {
      const kv = parseState((0, import_node_fs34.readFileSync)(statePath, "utf8"));
      phase = kv.phase ?? "";
    ...
    if (latest) {
      const r = readResultJson((0, import_node_path31.join)(expsDir, latest, "result.json"));
...
async function experimentSendWith(args, deps) {
  ...
  if (p.badArgs) {
    log.error("autoresearch experiment-send: usage: [--inputs csv] [--context-file path] [--smoke-test script] [--timeout N] [--parent exp-id] <topic> <agent> <exp-id> <approach-label> <approach-brief>");
    return 2;
  }
  const { topic, agent, expId, approachLabel, approachBrief } = p;
  if (!EXP_ID_RE.test(expId)) {
    log.error(`autoresearch experiment-send: exp-id must match exp-[0-9]+; got '${expId}'`);
    return 2;
  }
  if (!AGENT_RE.test(agent)) {
    log.error(`autoresearch experiment-send: agent must match [a-z][a-z0-9-]*; got '${agent}'`);
    return 2;
  }
  if (p.inputs) {
    for (const path6 of p.inputs.split(",")) {
      if (!path6) continue;
      try {
        (0, import_node_fs34.accessSync)(path6, import_node_fs34.constants.R_OK);
      } catch {
        log.error(`autoresearch experiment-send: cannot read input path '${path6}'`);
        return 2;
      }
    }
  }
  if (p.timeout !== void 0 && !/^[1-9][0-9]*$/.test(p.timeout)) {
    log.error(`autoresearch experiment-send: --timeout must be a positive integer (seconds); got '${p.timeout}'`);
    return 2;
  }
  if (p.smokeTest) {
    try {
      (0, import_node_fs34.accessSync)(p.smokeTest, import_node_fs34.constants.X_OK);
    } catch {
      log.error(`autoresearch experiment-send: smoke-test script not executable: ${p.smokeTest}`);
      return 2;
    }
  }
  let taskContext = "";
  if (p.contextFile) {
    try {
      taskContext = (0, import_node_fs34.readFileSync)(p.contextFile, "utf8");
    } catch {
      log.error(`autoresearch experiment-send: cannot read --context-file: ${p.contextFile}`);
      return 2;
    }
  }
  const art = autoresearchArtDir(topic, opts);
  if (!(0, import_node_fs34.existsSync)(art)) {
    log.error(`autoresearch experiment-send: topic state dir missing: ${art} (was autoresearch init run?)`);
    return 1;
  }
  const metricMd = (0, import_node_path31.join)(art, "metric.md");
  if (!(0, import_node_fs34.existsSync)(metricMd)) {
    log.error(`autoresearch experiment-send: metric.md missing at ${metricMd}`);
    return 1;
  }
  const stateDir = workerStateDir(art, agent);
  const stateTxt = (0, import_node_path31.join)(stateDir, "state.txt");
  if (!(0, import_node_fs34.existsSync)(stateTxt)) {
    log.error(`autoresearch experiment-send: worker state.txt missing: ${stateTxt}`);
    return 1;
  }
  const phase = parseState((0, import_node_fs34.readFileSync)(stateTxt, "utf8")).phase ?? "";
  if (phase === "abandoned") {
    log.error(`autoresearch experiment-send: worker ${agent} lane is abandoned; not dispatching`);
    return 2;
  }
  if (phase !== "idle") {
    log.error(`autoresearch experiment-send: worker ${agent} not idle (phase=${phase}); wait or finalize first`);
    return 1;
  }
  if (p.parentId !== void 0) {
    if (!EXP_ID_RE.test(p.parentId)) {
      log.error(`autoresearch experiment-send: --parent must match exp-[0-9]+; got '${p.parentId}'`);
      return 2;
    }
    if (!(0, import_node_fs34.existsSync)(experimentDir(art, agent, p.parentId))) {
      log.error(`autoresearch experiment-send: --parent ${p.parentId} has no experiment dir under ${agent}`);
      return 1;
    }
  }
  const branchDir = experimentDir(art, agent, expId);
  (0, import_node_fs34.mkdirSync)((0, import_node_path31.join)(branchDir, "code"), { recursive: true });
  if (p.smokeTest) {
    ...
    if (!r.ok) {
      ...
      log.error(`autoresearch experiment-send: smoke-test failed for ${agent}/${expId}; stderr -> ${(0, import_node_path31.join)(branchDir, "smoke-test.err")}`);
      return 2;
    }
  }
  const model = resolveModel(agent, topic);
  if (!model) {
    log.error(`autoresearch experiment-send: no worker '${agent}' on topic '${topic}' (resolveModel null)`);
    return 1;
  }
  const outbox = outboxPath(agent, model, topic);
  if (!(0, import_node_fs34.existsSync)(outbox)) {
    log.error(`autoresearch experiment-send: worker outbox missing: ${outbox} (was spawn run for ${agent}?)`);
    return 1;
  }
  const metricBlock = (0, import_node_fs34.readFileSync)(metricMd, "utf8");
  const metricName = parseMetricMd(metricBlock).primaryMetric;
  if (!metricName) {
    log.error(`autoresearch experiment-send: could not parse Primary metric from ${metricMd}`);
    return 1;
  }
  ...
  const templatePath = (0, import_node_path31.join)(pluginRoot(), "config", "prompt-templates", "autoresearch", "experiment.md");
  if (!(0, import_node_fs34.existsSync)(templatePath)) {
    log.error(`autoresearch experiment-send: template missing: ${templatePath}`);
    return 1;
  }
  const template = (0, import_node_fs34.readFileSync)(templatePath, "utf8");
  let prompt;
  try {
    ...
    log.error(`autoresearch experiment-send: ${e.message}`);
    return 1;
  }
  if (prompt.trim() === "") {
    log.error(`autoresearch experiment-send: prompt rendered empty (template substitution failed)`);
    return 1;
  }
  atomicWrite((0, import_node_path31.join)(branchDir, "prompt.md"), prompt);
  if (p.parentId !== void 0) atomicWrite((0, import_node_path31.join)(branchDir, "lineage.txt"), `parent_id=${p.parentId}
`);
  inboxWrite(agent, model, topic, prompt, { from: "hub", noDoneInstruction: true });
  atomicWrite(stateTxt, buildDispatchState((0, import_node_fs34.readFileSync)(stateTxt, "utf8"), expId, deps.now()));
  if (!deps.dryRun) {
    const pane = paneMetaRead(agent, model, topic);
    if (pane) {
      try {
...
async function scoreWith(args, deps) {
  ...
  if (!(0, import_node_fs34.existsSync)(workersRoot)) {
  ...
  deps.writeAtomic((0, import_node_path31.join)(art, "sanity.tsv"), SANITY_TSV_HEADER + c3.sanityRows.map(sanityRow).join(""));
  deps.writeAtomic((0, import_node_path31.join)(art, "coverage.tsv"), COVERAGE_TSV_HEADER + c3.coverageRows.map(coverageRow).join(""));
  deps.writeAtomic((0, import_node_path31.join)(art, "lineage.tsv"), LINEAGE_TSV_HEADER + c3.lineageRows.map(lineageRow).join(""));
...
async function monitorRun(args, opts) {
  ...
  const outbox = outboxPath(agent, model, topic);
  const stateDir = workerStateDir(art, agent);
  (0, import_node_fs34.mkdirSync)(stateDir, { recursive: true });
  ...
  const persist = (state2) => {
    (0, import_node_fs34.writeFileSync)(cursorFile, String(state2.offset));
    (0, import_node_fs34.writeFileSync)(rescanFile, [...state2.rescanEmitted].join("\n"));
  };
  const initBuf = (0, import_node_fs34.existsSync)(outbox) ? (0, import_node_fs34.readFileSync)(outbox) : Buffer.alloc(0);
  ...
  persist(state);
  do {
    const buf = (0, import_node_fs34.existsSync)(outbox) ? (0, import_node_fs34.readFileSync)(outbox) : Buffer.alloc(0);
    ...
    const mtime = (0, import_node_fs34.existsSync)(outbox) ? Math.floor((0, import_node_fs34.statSync)(outbox).mtimeMs / 1e3) : 0;
    const phase = ((0, import_node_fs34.existsSync)(stateTxt) ? parseState((0, import_node_fs34.readFileSync)(stateTxt, "utf8")).phase : "") ?? "";
...
function approachFromPrompt(promptPath) {
  if (!(0, import_node_fs34.existsSync)(promptPath)) return "";
  for (const line of (0, import_node_fs34.readFileSync)(promptPath, "utf8").split("\n")) {
...
function gatherCompletion(art) {
  ...
  const completion = scoreboardMd !== null && (0, import_node_fs34.existsSync)(metricPath) ? checkCompletion(scoreboardMd, (0, import_node_fs34.readFileSync)(metricPath, "utf8")) : null;
...
function parseStatusBriefArgs(args) {
  ...
  for (let i2 = 0; i2 < args.length; i2++) {
    const a2 = args[i2];
    if (a2 === "--latest-agent") latestAgent = args[++i2];
...
function readTsvRows(path6, headerToken) {
  if (!(0, import_node_fs34.existsSync)(path6)) return void 0;
  const rows = [];
  for (const line of (0, import_node_fs34.readFileSync)(path6, "utf8").split("\n")) {
...
async function statusBriefWith(args, v = {}) {
  ...
  const workersFile = (0, import_node_path31.join)(art, "workers.txt");
  if ((0, import_node_fs34.existsSync)(workersFile)) {
    const agents = splitNonCommentLines((0, import_node_fs34.readFileSync)(workersFile, "utf8"));
    for (const agent of agents) {
      let phase = "?", currentOrLast = "\u2014";
      const stateTxt = (0, import_node_path31.join)(workerStateDir(art, agent), "state.txt");
      let curExp = "";
      if ((0, import_node_fs34.existsSync)(stateTxt)) {
        const kv = parseState((0, import_node_fs34.readFileSync)(stateTxt, "utf8"));
        phase = kv.phase || "?";
      ...
      const promptPath = expForFiles ? (0, import_node_path31.join)(experimentDir(art, agent, expForFiles), "prompt.md") : "";
      const resultPath = expForFiles ? (0, import_node_path31.join)(experimentDir(art, agent, expForFiles), "result.json") : "";
      let approach, metric;
      if (phase === "working") {
  ...
  const vraw = readIfExistsOrNull((0, import_node_path31.join)(art, "verification.tsv"));
  const verdicts = vraw === null ? void 0 : parseVerdicts2(vraw);
  const srows = readTsvRows((0, import_node_path31.join)(art, "sanity.tsv"), "exp_id	");
  let suspects;
  if (srows) {
    suspects = {};
    for (const c3 of srows) if (c3[0] && c3[1] && c3[2]) (suspects[`${c3[1]}/${c3[0]}`] ??= []).push(c3[2]);
  }
  const crows = readTsvRows((0, import_node_path31.join)(art, "coverage.tsv"), "family	");
  let coverage;
  if (crows) {
    coverage = [];
    for (const cells of crows) if (cells[0]) coverage.push({ family: cells[0], count: parseInt(cells[1] ?? "0", 10) || 0, best: cells[2] ?? "", ts: cells[3] ?? "" });
  }
  const lrows = readTsvRows((0, import_node_path31.join)(art, "lineage.tsv"), "exp_id	");
  let multiChange;
  if (lrows) {
    multiChange = {};
    for (const cells of lrows) if (cells[0] && cells[1] && cells[4] === "improve-multi") multiChange[`${cells[1]}/${cells[0]}`] = true;
  }
  const iraw = readIfExistsOrNull((0, import_node_path31.join)(art, "inspection.tsv"));
...
function readOr(path6, fallback = "") {
  try {
    return (0, import_node_fs34.readFileSync)(path6, "utf8");
  } catch {
    return fallback;
  }
}
function listExpDirs(expsRoot) {
  try {
    return (0, import_node_fs34.readdirSync)(expsRoot, { withFileTypes: true }).filter((e) => e.isDirectory() && EXP_ID_RE.test(e.name)).map((e) => e.name).sort();
  } catch {
    return [];
  }
}
function dirByteSize(dir) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) total += dirByteSize(p);
      ...
      try {
        total += (0, import_node_fs34.statSync)(p).size;
      } catch {
      }
    }
  }
...
function fileCountDepth1(dir) {
  try {
    return (0, import_node_fs34.readdirSync)(dir, { withFileTypes: true }).filter((e) => e.isFile()).length;
  } catch {
    return 0;
  }
}
function normalizeResults(art, agents) {
  for (const agent of agents) {
    ...
    for (const expId of listExpDirs(expsRoot)) {
      ...
      if (!(0, import_node_fs34.existsSync)(resultPath)) continue;
      let parsed;
      try {
        parsed = JSON.parse((0, import_node_fs34.readFileSync)(resultPath, "utf8"));
      } catch {
        continue;
      }
...
function pruneIntermediate(art, agents) {
  for (const agent of agents) {
    ...
    for (const expId of listExpDirs(expsRoot)) {
      ...
      let keptRel;
      try {
        const r = JSON.parse((0, import_node_fs34.readFileSync)(resultPath, "utf8"));
      ...
      let entries;
      try {
        entries = (0, import_node_fs34.readdirSync)(expDir);
      } catch {
        continue;
      }
      for (const name of entries) {
        ...
        try {
          if ((0, import_node_fs34.statSync)(pt).isFile()) (0, import_node_fs34.rmSync)(pt, { force: true });
        } catch {
        }
      }
    }
...
function linkPaneArtifacts(art, agents, topic) {
  for (const agent of agents) {
    ...
    (0, import_node_fs34.mkdirSync)(targetDir, { recursive: true });
    const paneFiles = [
    ...
    for (const [name, src] of paneFiles) {
      ...
      try {
        try {
          if ((0, import_node_fs34.lstatSync)(linkPath)) (0, import_node_fs34.unlinkSync)(linkPath);
        } catch {
        }
        (0, import_node_fs34.symlinkSync)(rel, linkPath);
      } catch {
      }
    }
  }
...
function computeAuditWarnings(art, agents, warningsPath) {
  ...
  for (const agent of agents) {
    ...
    for (const expId of listExpDirs(expsRoot)) {
      ...
      if (!(0, import_node_fs34.existsSync)(promptMd) || !(0, import_node_fs34.existsSync)(auditJson)) continue;
      let audit;
      try {
        audit = JSON.parse((0, import_node_fs34.readFileSync)(auditJson, "utf8"));
      } catch {
        continue;
      }
      for (const { key, value } of parseHardConstraints((0, import_node_fs34.readFileSync)(promptMd, "utf8"))) {
...
function writeFinalizeLessons(art, agents, deps) {
  try {
    const thresholds = parseMetricMd(readOr((0, import_node_path31.join)(art, "metric.md")));
    const family = metricFamilyOf(thresholds.primaryMetric);
    if (!family) return;
    const direction = thresholds.direction ?? "maximize";
    const a1 = parseVerdicts2(readOr((0, import_node_path31.join)(art, "verification.tsv")));
    const c1 = parseInspections(readOr((0, import_node_path31.join)(art, "inspection.tsv")));
    const now = deps.now();
    const drafts = [];
    const verdicts = [];
    for (const agent of agents) {
      ...
      for (const expId of listExpDirs(expsRoot)) {
        ...
        let r;
        try {
          r = JSON.parse((0, import_node_fs34.readFileSync)(resultPath, "utf8"));
        } catch {
          continue;
        }
        ...
        const parentId = (parseState(readOr((0, import_node_path31.join)(expDir, "lineage.txt"))).parent_id ?? "").trim();
        if (parentId) {
          try {
            const pr = JSON.parse((0, import_node_fs34.readFileSync)((0, import_node_path31.join)(expsRoot, parentId, "result.json"), "utf8"));
...
async function finalizeWith(args, deps) {
  ...
  const topic = rest[0];
  const art = autoresearchArtDir(topic, opts);
  if (!(0, import_node_fs34.existsSync)(art) || !(0, import_node_fs34.statSync)(art).isDirectory()) {
    log.error(`finalize: art-dir missing: ${art}`);
    return 1;
  }
  const workersFile = (0, import_node_path31.join)(art, "workers.txt");
  const agents = (0, import_node_fs34.existsSync)(workersFile) ? splitNonCommentLines((0, import_node_fs34.readFileSync)(workersFile, "utf8")) : [];
  for (const agent of agents) {
    const stateDir = workerStateDir(art, agent);
    const stateTxt = (0, import_node_path31.join)(stateDir, "state.txt");
    if (!(0, import_node_fs34.existsSync)(stateTxt)) continue;
    ...
    let tail = "";
    if (ob && (0, import_node_fs34.existsSync)(ob)) {
      try {
        tail = (0, import_node_fs34.readFileSync)(ob).subarray(offset).toString("utf8");
      } catch {
        tail = "";
      }
    }
    const curExp = parseState(readOr(stateTxt)).current_exp_id ?? "";
    const doneResultExists = !!curExp && (0, import_node_fs34.existsSync)((0, import_node_path31.join)(experimentDir(art, agent, curExp), "result.json"));
  ...
  const sanityExtra = [];
  for (const c3 of readTsvRows((0, import_node_path31.join)(art, "sanity.tsv"), "exp_id	") ?? []) {
    if (c3[2] === "audit-knob-drift") continue;
    if (c3[0] && c3[1] && c3[2]) sanityExtra.push(`sanity	${c3[1]}/${c3[0]}	${c3[2]}	${c3[3] ?? ""}`);
  }
  if (sanityExtra.length) (0, import_node_fs34.appendFileSync)(warningsPath, sanityExtra.join("\n") + "\n");
  const lineageExtra = [];
  for (const c3 of readTsvRows((0, import_node_path31.join)(art, "lineage.tsv"), "exp_id	") ?? []) {
  ...
  if (lineageExtra.length) (0, import_node_fs34.appendFileSync)(warningsPath, lineageExtra.join("\n") + "\n");
  const reimplExtra = [];
  for (const c3 of readTsvRows((0, import_node_path31.join)(art, "inspection.tsv"), "exp_id	") ?? []) {
    if (c3[2] !== "not-reproduced") continue;
    if (c3[0] && c3[1]) reimplExtra.push(`reimpl	${c3[1]}/${c3[0]}	not-reproduced	${c3[3] ?? ""}`);
  }
  if (reimplExtra.length) (0, import_node_fs34.appendFileSync)(warningsPath, reimplExtra.join("\n") + "\n");
  const statusRows = [];
  for (const agent of agents) {
    const stateTxt = (0, import_node_path31.join)(workerStateDir(art, agent), "state.txt");
    if ((0, import_node_fs34.existsSync)(stateTxt)) {
  ...
  let hardCap = null;
  if ((0, import_node_fs34.existsSync)(budgetPath) && (0, import_node_fs34.existsSync)(startPath)) {
    try {
      hardCap = checkTimeBudget(
        (0, import_node_fs34.readFileSync)(budgetPath, "utf8").trim(),
        (0, import_node_fs34.readFileSync)(startPath, "utf8").trim(),
  ...
  for (const agent of agents) {
    ...
    if (!(0, import_node_fs34.existsSync)(ob)) continue;
    const lines = readOr(ob).split("\n").filter((l) => l.trim() !== "").slice(-10);
    for (const line of lines) {
      try {
        const o2 = JSON.parse(line);
        allEvents.push({ ts: o2.ts != null ? String(o2.ts) : "", agent, event: o2.event != null ? String(o2.event) : "" });
      } catch {
      }
    }
  }
  ...
  const startedIso = (0, import_node_fs34.existsSync)(startPath) ? (0, import_node_fs34.readFileSync)(startPath, "utf8").trim() : "(unknown)";
  const budget = (0, import_node_fs34.existsSync)(budgetPath) ? (0, import_node_fs34.readFileSync)(budgetPath, "utf8").trim() : "none";
...
async function refineWith(args, deps) {
  ...
  const art = autoresearchArtDir(topic, deps.opts);
  const branchDir = experimentDir(art, agent, expId);
  if (!(0, import_node_fs34.existsSync)(branchDir) || !(0, import_node_fs34.statSync)(branchDir).isDirectory()) {
    log.error(`branch dir missing: ${branchDir}`);
    return 1;
  }
  let n2 = 1;
  while ((0, import_node_fs34.existsSync)((0, import_node_path31.join)(branchDir, `refine-${n2}.md`))) n2++;
  ...
  if (!deps.dryRun) {
    ...
    try {
      ...
      if (rc !== 0) log.warn(`[refine] send nudge failed; worker may not have noticed refine-${n2}.md`);
    } catch {
      log.warn(`[refine] send nudge failed; worker may not have noticed refine-${n2}.md`);
    }
  }
  log.ok(`[refine] ${agent}/${expId} refine-${n2}.md sent`);
  return 0;
}
function readResultJson(path6) {
  if (!(0, import_node_fs34.existsSync)(path6)) return {};
  try {
    return JSON.parse((0, import_node_fs34.readFileSync)(path6, "utf8"));
  } catch {
    return {};
  }
}
...
async function handoffExtractWith(args, deps) {
  const art = args[0];
  if (!art || !(0, import_node_fs34.existsSync)(art) || !(0, import_node_fs34.statSync)(art).isDirectory()) {
    log.error(`autoresearch handoff-extract: art-dir required (got '${art ?? ""}')`);
    return 2;
  }
  const topicTxt = (0, import_node_path31.join)(art, "topic.txt");
  if (!(0, import_node_fs34.existsSync)(topicTxt)) {
    log.error(`autoresearch handoff-extract: topic.txt missing under ${art}`);
    return 2;
  }
  const topic = (0, import_node_fs34.readFileSync)(topicTxt, "utf8").replace(/\n/g, " ").replace(/\s+$/, "");
  const sbPath = (0, import_node_path31.join)(art, "scoreboard.md");
  const { winner, runnerUps } = parseScoreboard(readIfExists(sbPath));
  let landscapeDoc;
  for (const name of (0, import_node_fs34.readdirSync)(art).sort()) {
    if (/^autoresearch-.*\.md$/.test(name) && (0, import_node_fs34.statSync)((0, import_node_path31.join)(art, name)).isFile()) {
      landscapeDoc = name;
      break;
    }
  }
  const hasMetricMd = (0, import_node_fs34.existsSync)((0, import_node_path31.join)(art, "metric.md"));
  ...
  if (!winner) {
    ...
    const runners = runnerUps.map((r) => {
      const rr = readResultJson((0, import_node_path31.join)(art, `workers/${r.agent}/experiments/${r.expId}`, "result.json"));
...
function sweepTmpLock(dir, depth) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      ...
      try {
        (0, import_node_fs34.rmSync)(p, { force: true });
      } catch {
      }
    }
  }
...
async function teardownWith(args, deps) {
  ...
  if ((0, import_node_fs34.existsSync)(pf)) {
    for (const pane of parsePanesFile((0, import_node_fs34.readFileSync)(pf, "utf8")).values()) {
      try {
    ...
    try {
      (0, import_node_fs34.rmSync)(pf, { force: true });
    } catch {
    }
  }
  if (panesOnly) {
    try {
      (0, import_node_fs34.rmSync)((0, import_node_path31.join)(art, "spawn-results.tsv"), { force: true });
    } catch {
    }
  ...
  if ((0, import_node_fs34.existsSync)(shared) && (0, import_node_fs34.statSync)(shared).isDirectory()) sweepTmpLock(shared, 2);
  const sbPath = (0, import_node_path31.join)(art, "scoreboard.md");
  if ((0, import_node_fs34.existsSync)(sbPath)) {
    const { winner } = parseScoreboard((0, import_node_fs34.readFileSync)(sbPath, "utf8"));
    if (winner) {
      const rel = `workers/${winner.agent}/experiments/${winner.expId}/code`;
      if ((0, import_node_fs34.existsSync)((0, import_node_path31.join)(art, rel)) && (0, import_node_fs34.statSync)((0, import_node_path31.join)(art, rel)).isDirectory()) {
        const link = (0, import_node_path31.join)(art, "winner");
        try {
          (0, import_node_fs34.rmSync)(link, { force: true });
        } catch {
        }
        (0, import_node_fs34.symlinkSync)(rel, link);
...
async function freshWorkerWith(args, deps) {
  ...
  const prev = parseState((0, import_node_fs34.readFileSync)(stateTxt, "utf8"));
  ...
  atomicWrite(stateTxt, mergeState((0, import_node_fs34.readFileSync)(stateTxt, "utf8"), {
...
async function abortWith(args, deps) {
  ...
  const ids = (0, import_node_fs34.existsSync)(mt) ? (0, import_node_fs34.readFileSync)(mt, "utf8").split("\n").map((l) => l.trim()).filter(Boolean) : [];
  (0, import_node_fs34.writeFileSync)((0, import_node_path31.join)(art, "halt.flag"), `halted_by=user
halted_at=${deps.now()}
reason=${reason}
`);
...
async function consensusWith(args, deps) {
  ...
  let agents;
  try {
    agents = (0, import_node_fs34.readdirSync)(workersRoot, { withFileTypes: true }).filter((e) => e.isDirectory()).map((e) => e.name).sort();
  } catch {
    agents = [];
  }
  for (const agent of agents) {
    ...
    let names;
    try {
      names = (0, import_node_fs34.readdirSync)(expsRoot).filter((n2) => EXP_ID_RE.test(n2)).sort();
    } catch {
      continue;
    }
    let newest = "";
    for (const exp of names) {
      const resultPath = (0, import_node_path31.join)(experimentDir(art, agent, exp), "result.json");
      if (!(0, import_node_fs34.existsSync)(resultPath)) continue;
      let parsed;
      try {
        parsed = JSON.parse((0, import_node_fs34.readFileSync)(resultPath, "utf8"));
...
function appendVerificationRow(art, agent, expId, row) {
  ...
  atomicWrite(
    (0, import_node_path31.join)(experimentDir(art, agent, expId), "verification.txt"),
    `${row.verdict} reason=${row.reason} recomputed=${row.recomputed} at ${row.ts}
`
  );
}
function appendInspectionRow(art, agent, expId, row) {
  const tsv = (0, import_node_path31.join)(art, "inspection.tsv");
  const prior = (0, import_node_fs34.existsSync)(tsv) ? (0, import_node_fs34.readFileSync)(tsv, "utf8") : INSPECTION_TSV_HEADER;
  atomicWrite(tsv, prior + inspectionRow(row));
  atomicWrite(
    (0, import_node_path31.join)(experimentDir(art, agent, expId), "inspection.txt"),
...
var init_autoresearch2 = __esm({
  "src/commands/autoresearch.ts"() {
    ...
    liveScoreDeps = {
      computeScore,
      fs: {
        exists: import_node_fs34.existsSync,
        read: readIfExistsOrNull,
        listDir: (p) => {
          try {
            return (0, import_node_fs34.readdirSync)(p).sort();
          } catch {
            return [];
          }
        }
    ...
    liveVerifyPlanDeps = {
      ...
      readManifest: (art, i2, e) => {
        const p = (0, import_node_path31.join)(experimentDir(art, i2, e), "verify-manifest.json");
        if (!(0, import_node_fs34.existsSync)(p)) return null;
        try {
          return JSON.parse((0, import_node_fs34.readFileSync)(p, "utf8"));
        } catch {
      ...
      readInput: (art, i2, e, rel) => {
        const p = (0, import_node_path31.join)(experimentDir(art, i2, e), rel);
        return readIfExistsOrNull(p);
      },
    ...
    liveVerifyCheckDeps = {
      ...
      readMetricMd: (art) => readIfExistsOrNull((0, import_node_path31.join)(art, "metric.md")),
    ...
    liveInspectPlanDeps = {
      ...
      readMetricMd: (art) => readIfExistsOrNull((0, import_node_path31.join)(art, "metric.md")),
      inspectionCount: (art) => {
        const p = (0, import_node_path31.join)(art, "inspection.tsv");
        if (!(0, import_node_fs34.existsSync)(p)) return 0;
        return (0, import_node_fs34.readFileSync)(p, "utf8").split("\n").filter((l) => l && !l.startsWith("exp_id	")).length;
      },
...
function exploreArtDir(topic, opts) {
  return (0, import_node_path32.join)(topicDir(topic, opts), "_explore");
}
...
function buildHandoffKv2(i2) {
  const L = [];
  L.push(`mode=${i2.topApproach ? "explore" : "explore-no-convergence"}`);
...
function extractHandoffData(artDir, now) {
  if (!(0, import_node_fs35.existsSync)(artDir) || !(0, import_node_fs35.statSync)(artDir).isDirectory()) return null;
  const topicTxt = readIfExistsOrNull((0, import_node_path33.join)(artDir, "topic.txt"));
  if (topicTxt === null) return null;
  const topic = topicTxt.replace(/\n/g, " ").replace(/ +$/, "");
  const names = (0, import_node_fs35.readdirSync)(artDir);
  ...
  if (landscapeDoc) {
    const doc = (0, import_node_fs35.readFileSync)((0, import_node_path33.join)(artDir, landscapeDoc), "utf8");
    top = topApproach(doc);
    tradeoff = /^## Tradeoff matrix/m.test(doc);
  }
  let confidenceSignals = "";
  const skip = readIfExistsOrNull((0, import_node_path33.join)(artDir, "adversary-skip.txt"));
  if (skip) {
...
function buildAnnotations(draft, findings) {
  ...
  for (let i2 = 0; i2 < lines.length; i2++) {
    for (const tok of solo) {
      ...
      const replaced = lines[i2].replace(re, tok + " [unverified]");
      if (replaced !== lines[i2]) {
        lines[i2] = replaced;
...
async function initWith5(tokens, d) {
  ...
  atomicWrite((0, import_node_path34.join)(art, "list.txt"), formatListFile(rows, isoUtc()));
...
async function classifyRun(rest) {
  ...
  const topicText = readIfExists((0, import_node_path34.join)(art, "topic.txt")).trim();
...
async function researchSendRun2(rest) {
  const [topic, agent, provider] = rest;
  if (!topic || !agent || !provider) {
    log.error("usage: explore research-send <topic> <agent> <provider>");
    return 2;
  }
  return researchSendWith2(topic, agent, provider, liveResearchSendDeps2);
}
async function researchSendWith2(topic, agent, provider, d) {
  const art = exploreArtDir(topic);
  const stateFile = (0, import_node_path34.join)(art, `research-${agent}.txt`);
...
`);
  const rc = await d.send(["--from", "hub", agent, topic, `@${promptFile}`]);
  if (rc !== 0) {
    log.error(`explore research-send: send failed (rc=${rc}); ${stateFile} kept (rm to redo)`);
    return 1;
  }
...
async function researchWaitRun2(rest) {
  const [topic, agent, provider] = rest;
  if (!topic || !agent || !provider) {
    log.error("usage: explore research-wait <topic> <agent> <provider>");
    return 2;
  }
  return researchWaitWith2(topic, agent, provider, liveResearchWaitDeps2);
}
async function researchWaitWith2(topic, agent, provider, d) {
  const art = exploreArtDir(topic);
  const stateFile = (0, import_node_path34.join)(art, `research-${agent}.txt`);
  if (!(0, import_node_fs36.existsSync)(stateFile)) {
    log.error(`explore research-wait: ${stateFile} missing (run explore research-send first)`);
    return 1;
  }
  const offset = parseLatestOffset((0, import_node_fs36.readFileSync)(stateFile, "utf8"));
  if (offset === null) {
    log.error(`explore research-wait: OFFSET not set in ${stateFile}`);
    return 1;
  }
  const timeout = scaledTimeout(consultTimeout("research"), d.multiplier(provider));
  ...
  const fs = researchState(ev, findingsText);
  if (fs === "question" && ev) {
    atomicWrite((0, import_node_path34.join)(art, `question-${agent}.txt`), JSON.stringify(ev) + "\n");
    const bumped = outboxOffset(outboxPath(agent, provider, topic));
    (0, import_node_fs36.appendFileSync)(stateFile, `OFFSET=${bumped}
FS=question
`);
  } else {
    (0, import_node_fs36.appendFileSync)(stateFile, `FS=${fs}
`);
  }
  (0, import_node_fs36.writeFileSync)((0, import_node_path34.join)(art, `research-${agent}.done`), "");
...
async function synthPreliminaryRun(rest) {
  ...
  for (const f of ["topic.txt", "list.txt"]) {
    if (!readIfExists((0, import_node_path34.join)(art, f)).trim()) {
      log.error(`explore synth-preliminary: missing or empty: ${(0, import_node_path34.join)(art, f)}`);
      return 1;
    }
  }
...
async function confidenceRun(rest) {
  ...
  const art = exploreArtDir(topic);
  const draft = readIfExists((0, import_node_path34.join)(art, "landscape-draft.md"));
  if (!draft.trim()) {
...
async function annotateRun(rest) {
  ...
  const art = exploreArtDir(topic);
  const markerPath = (0, import_node_path34.join)(art, "annotate-applied.txt");
  if ((0, import_node_fs36.existsSync)(markerPath)) {
    log.ok(`explore annotate: already applied (${markerPath}) \u2014 no-op`);
    return 0;
  }
  const draftPath = (0, import_node_path34.join)(art, "landscape-draft.md");
  ...
  atomicWrite((0, import_node_path34.join)(art, "annotations.json"), JSON.stringify({ topic, counts, items: plan.items }, null, 2) + "\n");
  atomicWrite(
    markerPath,
    `applied: ${isoUtc()}
...
async function adversarySendRun(rest) {
  const [topic, agent, provider] = rest;
  if (!topic || !agent || !provider) {
    log.error("usage: explore adversary-send <topic> <agent> <provider>");
    return 2;
  }
  return adversarySendWith(topic, agent, provider, liveResearchSendDeps2);
}
async function adversarySendWith(topic, agent, provider, d) {
  ...
  const stateFile = (0, import_node_path34.join)(art, `adversary-${agent}.txt`);
  if ((0, import_node_fs36.existsSync)(stateFile)) {
...
`);
  const rc = await d.send(["--from", "hub", agent, topic, `@${promptFile}`]);
  if (rc !== 0) {
    log.error(`explore adversary-send: send failed (rc=${rc}); ${stateFile} kept (rm to redo)`);
    return 1;
  }
...
async function adversaryWaitRun(rest) {
  const [topic, agent, provider] = rest;
  if (!topic || !agent || !provider) {
    log.error("usage: explore adversary-wait <topic> <agent> <provider>");
    return 2;
  }
  return adversaryWaitWith(topic, agent, provider, liveResearchWaitDeps2);
}
...
`);
    ...
    (0, import_node_fs36.appendFileSync)(stateFile, `AS=${as}
`);
  }
  (0, import_node_fs36.writeFileSync)((0, import_node_path34.join)(art, `adversary-${agent}.done`), "");
...
async function exploreWaitGateRun(rest) {
  ...
  const listPath = (0, import_node_path34.join)(art, "list.txt");
  if (!(0, import_node_fs36.existsSync)(listPath)) {
    log.error(`explore wait-gate: list.txt missing at ${art}`);
    return 2;
  }
  ...
  const workers = rows.map((r) => {
    ...
    return {
      agent: r.agent,
      doneExists: (0, import_node_fs36.existsSync)((0, import_node_path34.join)(art, `${phase}-${r.agent}.done`)),
      stateText: readIfExistsOrNull(stateFile)
    };
  });
  const states = gateState(workers, key);
...
async function synthFinalRun(rest) {
  ...
  if (!readIfExists((0, import_node_path34.join)(art, "landscape-draft.md")).trim()) {
    log.error("explore synth-final: landscape-draft.md missing");
    return 1;
  }
  if (!readIfExists((0, import_node_path34.join)(art, "topic.txt")).trim()) {
...
async function teardownWith2(args, deps) {
  ...
  const art = exploreArtDir(topic);
  if (!(0, import_node_fs36.existsSync)(art) || !(0, import_node_fs36.statSync)(art).isDirectory()) {
    log.error(`${art} not found`);
    return 1;
  }
  const pf = (0, import_node_path34.join)(art, "preflight-panes.txt");
  if ((0, import_node_fs36.existsSync)(pf)) {
    for (const pane of parsePanesFile((0, import_node_fs36.readFileSync)(pf, "utf8")).values()) {
      try {
        await deps.killPane(pane);
      } catch {
      }
  ...
  if (panesOnly) {
    for (const f of ["preflight-panes.txt", "spawn-results.tsv"]) {
      try {
        (0, import_node_fs36.rmSync)((0, import_node_path34.join)(art, f), { force: true });
      } catch {
      }
    }
    log.ok(`[teardown] panes-only reset for ${topic} (state preserved for retry)`);
...
async function handoffExtractRun(rest) {
  ...
  log.ok(`explore handoff-extract: wrote ${path6}`);
  process.stdout.write(path6 + "\n");
  return 0;
}
var import_node_fs36, import_node_path34, liveExploreInitDeps, liveExploreSpawnAllDeps, liveResearchSendDeps2, liveResearchWaitDeps2, liveExploreTeardownDeps;
...
function bridgeArtDir(topic) {
  return (0, import_node_path35.join)(topicDir(topic), "_bridge");
}
function bridgeExecDir(topic) {
  return (0, import_node_path35.join)(bridgeArtDir(topic), "execute");
}
...
async function initWith6(tokens, d) {
  ...
  if (!(0, import_node_fs37.existsSync)(repo)) {
  ...
  (0, import_node_fs37.mkdirSync)(exec, { recursive: true });
  atomicWrite((0, import_node_path36.join)(art, "topic.txt"), slug + "\n");
  atomicWrite((0, import_node_path36.join)(art, "topic-text.txt"), taskText);
  atomicWrite((0, import_node_path36.join)(art, "selected-provider.txt"), provider + "\n");
  atomicWrite((0, import_node_path36.join)(art, "agent.txt"), agent + "\n");
  atomicWrite((0, import_node_path36.join)(art, "timing.txt"), `started=${isoUtc()}
`);
  atomicWrite((0, import_node_path36.join)(exec, "provider.txt"), provider + "\n");
  atomicWrite((0, import_node_path36.join)(exec, "mode.txt"), mode + "\n");
  atomicWrite((0, import_node_path36.join)(exec, "target_cwd.txt"), repo + "\n");
  atomicWrite((0, import_node_path36.join)(exec, "repo-b-head.txt"), (inPlace ? "" : d.headSha(repo)) + "\n");
...
async function branchRun3(rest) {
  ...
  const target = readField((0, import_node_path36.join)(bridgeExecDir(topic), "target_cwd.txt"));
  if (!target) {
...
async function branchWith3(topic, target, r) {
  ...
  atomicWrite((0, import_node_path36.join)(exec, "start-branch.txt"), snap.branch + "\n");
  atomicWrite((0, import_node_path36.join)(exec, "branch-base.sha"), snap.baseSha + "\n");
  atomicWrite((0, import_node_path36.join)(exec, "branch.txt"), branch + "\n");
  if (!onBranch) {
...
async function roundSendWith(topic, round, d) {
  ...
  const exec = bridgeExecDir(topic);
  const agent = readField((0, import_node_path36.join)(art, "agent.txt"));
  const provider = readField((0, import_node_path36.join)(art, "selected-provider.txt"));
  if (!agent || !provider) {
    log.error("bridge round-send: missing agent.txt/selected-provider.txt (run bridge init)");
    return 1;
  }
  const outbox = outboxPath(agent, provider, topic);
  if (!(0, import_node_fs37.existsSync)(outbox)) {
    log.error(`bridge round-send: outbox not found at ${outbox} \u2014 was ${agent} spawned?`);
    return 1;
  }
  const busy = workerBusyState(agent, provider, topic);
  if (busy) {
    log.error(`bridge round-send: worker not idle (state=${busy}); previous round still in flight`);
    return 1;
  }
  const stateFile = (0, import_node_path36.join)(exec, `round-${round}.txt`);
  if ((0, import_node_fs37.existsSync)(stateFile)) {
    log.error(`bridge round-send: ${stateFile} already exists; rm to retry`);
    return 1;
  }
  let prompt;
  if (round === 1) {
    const task = readIfExists((0, import_node_path36.join)(art, "topic-text.txt"));
    const repo = readField((0, import_node_path36.join)(exec, "target_cwd.txt"));
    const branch = readField((0, import_node_path36.join)(exec, "branch.txt")) || "the current branch";
    prompt = composeBridgeBrief(task, repo, branch);
  } else {
    const bundle = (0, import_node_path36.join)(exec, `followup-${round}.md`);
    if (!(0, import_node_fs37.existsSync)(bundle)) {
      log.error(`bridge round-send: follow-up bundle missing: ${bundle} (the directive must write it first)`);
      return 1;
    }
    prompt = composeBridgeFollowup((0, import_node_fs37.readFileSync)(bundle, "utf8"), round);
  }
  const promptFile = (0, import_node_path36.join)(exec, `round-prompt-${round}.md`);
  atomicWrite(promptFile, prompt);
  const offset = d.offsetFor(agent, provider, topic);
  atomicWrite(stateFile, `OFFSET=${offset}
`);
  const rc = await d.send([agent, topic, `@${promptFile}`]);
  if (rc !== 0) {
    log.error(`bridge round-send: send failed (rc=${rc}); ${stateFile} kept for retry`);
    return 1;
  }
...
async function roundWaitWith(topic, round, d) {
  ...
  const ts = classifyTurn(ev);
  if (ts === "question" && ev) {
    atomicWrite((0, import_node_path36.join)(exec, `question-${round}.txt`), JSON.stringify(ev) + "\n");
    const bumped = outboxOffset(outboxPath(agent, provider, topic));
    (0, import_node_fs37.appendFileSync)(stateFile, `OFFSET=${bumped}
TS=question
`);
  } else {
    (0, import_node_fs37.appendFileSync)(stateFile, `TS=${ts}
`);
  }
...
async function relayRun(rest) {
  ...
  (0, import_node_fs37.appendFileSync)((0, import_node_path36.join)(bridgeExecDir(topic), `question-${round}.txt`), `RELAYED=${answer}
`);
...
async function finishWith3(topic, r, hasGh) {
  const exec = bridgeExecDir(topic);
  const mode = readField((0, import_node_path36.join)(exec, "mode.txt")) || "branch";
  if (mode === "in-place") {
    atomicWrite((0, import_node_path36.join)(exec, "finish-result.txt"), "none	in-place (commits on the current branch)\n");
    log.ok("bridge finish: in-place \u2014 commits left on the current branch");
    return 0;
  }
  const branch = readField((0, import_node_path36.join)(exec, "branch.txt"));
  const startBranch = readField((0, import_node_path36.join)(exec, "start-branch.txt")) || "main";
  const base = readField((0, import_node_path36.join)(exec, "branch-base.sha"));
  if (base) {
    const ds = shortstat(r, base);
    atomicWrite((0, import_node_path36.join)(exec, "diff-stats.txt"), (ds || "(no changes)") + "\n");
  }
  const task = readIfExists((0, import_node_path36.join)(bridgeArtDir(topic), "topic-text.txt"));
  const verify = readField((0, import_node_path36.join)(exec, "verify-result.txt"));
...
(Automated bridge branch \u2014 merged into ${startBranch}.)`
  ...
  atomicWrite((0, import_node_path36.join)(exec, "finish-result.txt"), `${res.action}	${res.outcome}
`);
...
async function summaryRun3(rest) {
  ...
  const art = bridgeArtDir(topic);
  const exec = bridgeExecDir(topic);
  const started = kvField((0, import_node_path36.join)(art, "timing.txt"), "started") || "unknown";
  let ended, duration;
  const i2 = rest.indexOf("--aborted");
  const aborted2 = i2 >= 0;
  if (!aborted2) {
    ...
    const s = Date.parse(started), e = Date.parse(ended);
    duration = Number.isFinite(s) && Number.isFinite(e) ? Math.round((e - s) / 1e3) : 0;
    atomicWrite((0, import_node_path36.join)(art, "timing.txt"), `started=${started}
ended=${ended}
duration=${duration}
`);
  }
  let rounds = 0;
  while ((0, import_node_fs37.existsSync)((0, import_node_path36.join)(exec, `round-${rounds + 1}.txt`))) rounds++;
  const facts = {
    topic,
    status: aborted2 ? "aborted" : "ok",
    started,
    ended,
    duration,
    provider: readField((0, import_node_path36.join)(art, "selected-provider.txt")) || "unknown",
    agent: readField((0, import_node_path36.join)(art, "agent.txt")) || "unknown",
    repo: readField((0, import_node_path36.join)(exec, "target_cwd.txt")) || "<repo>",
    mode: readField((0, import_node_path36.join)(exec, "mode.txt")) || "branch",
    branch: readField((0, import_node_path36.join)(exec, "branch.txt")) || "(none)",
    rounds,
    verify: readField((0, import_node_path36.join)(exec, "verify-result.txt")) || "unknown",
    diffStats: readField((0, import_node_path36.join)(exec, "diff-stats.txt")) || "unknown",
    archived: readField((0, import_node_path36.join)(art, "archived-path.txt")) || "(not archived)",
    finishResult: readField((0, import_node_path36.join)(exec, "finish-result.txt")) || "(not finished)",
    abortedPhase: aborted2 ? rest[i2 + 1] : void 0,
    abortedGate: aborted2 ? rest[i2 + 2] : void 0,
    abortedReason: aborted2 ? rest.slice(i2 + 3).join(" ") || "unknown" : void 0
  };
  atomicWrite((0, import_node_path36.join)(art, "SUMMARY.md"), renderBridgeSummary(facts));
  if (aborted2) {
    atomicWrite((0, import_node_path36.join)(art, "RESUME.md"), renderBridgeResume({
      topic,
      repo: facts.repo,
      branch: facts.branch,
      mode: facts.mode,
      lastRound: rounds,
      task: readIfExists((0, import_node_path36.join)(art, "topic-text.txt")),
      phase: facts.abortedPhase ?? "unknown",
      gate: facts.abortedGate ?? "unknown"
    }));
  }
  log.ok(`bridge summary: wrote ${(0, import_node_path36.join)(art, "SUMMARY.md")}`);
  return 0;
}
var import_node_fs37, import_node_path36, liveInitDeps5, DUET_TURN_TIMEOUT;
...
async function loadHandlers() {
  const [spawn2, send, collect, list, stop, check, preflight, hook, quick, design, implement, review, autoresearch, explore, bridge] = await Promise.all([
    ...
    Promise.resolve().then(() => (init_stop(), stop_exports)),
    Promise.resolve().then(() => (init_check(), check_exports)),
    Promise.resolve().then(() => (init_preflight(), preflight_exports)),
    Promise.resolve().then(() => (init_hook(), hook_exports)),
    Promise.resolve().then(() => (init_quick2(), quick_exports)),
    Promise.resolve().then(() => (init_design2(), design_exports)),
    Promise.resolve().then(() => (init_implement2(), implement_exports)),
    Promise.resolve().then(() => (init_review2(), review_exports)),
    Promise.resolve().then(() => (init_autoresearch2(), autoresearch_exports)),
    Promise.resolve().then(() => (init_explore2(), explore_exports)),
    Promise.resolve().then(() => (init_bridge2(), bridge_exports))
  ]);
  return {
    spawn: spawn2.run,
    send: send.run,
