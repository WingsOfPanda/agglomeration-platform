#!/usr/bin/env bash
# Simulated-workers end-to-end dogfood for /ap:explore (port of meditate).
#
# Drives the REAL built CLI (`node dist/ap.cjs explore <verb>`) across the full
# explore lifecycle against a throwaway AP_HOME. The model PARTS are SIMULATED:
...
AP_HOME="$(mktemp -d "${TMPDIR:-/tmp}/ap-explore-dogfood.XXXXXX")"
export AP_HOME
trap 'rm -rf "$AP_HOME"' EXIT

# 2 consult-validated providers in the active list -> explore init picks N=2.
...
echo "===================================================================="
echo "explore simulated-workers dogfood — TOPIC: $TOPIC_TEXT"
echo "===================================================================="

# --- Step 1: init -----------------------------------------------------------
ARGS="$($CS explore --mint-args-file)"
printf 'attention kernels for long context' > "$ARGS"
OUT="$($CS explore init --args-file "$ARGS")"; rc=$?
...
TOPIC="$(printf '%s\n' "$OUT" | sed -n 's/^TOPIC=//p')"
ART="$(printf '%s\n' "$OUT" | sed -n 's/^ART=//p')"
assert "1d topic.txt + list.txt exist" \
  "$([ -n "$ART" ] && [ -f "$ART/topic.txt" ] && [ -f "$ART/list.txt" ] && echo 0 || echo 1)"

# --- Step 2: classify (topic has 'attention' -> lit-track ON) ---------------
$CS explore classify "$TOPIC" >/dev/null 2>&1; rc=$?
...
# --- Step 3: simulate research findings (one per agent in list) -------
AGENTS="$(awk -F'\t' '!/^#/ && NF>=2 {print $2}' "$ART/list.txt")"
assert "3a list yields >=2 agents" \
...
DRAFT_PATH="$($CS explore synth-preliminary "$TOPIC")"; rc=$?
...
CONF="$($CS explore confidence "$TOPIC")"; rc=$?
...
$CS explore confidence "$TOPIC" --decision continue >/dev/null 2>&1; rc=$?
...
FINAL_PATH="$($CS explore synth-final "$TOPIC")"; rc=$?
...
$CS explore forensics "$TOPIC" >/dev/null 2>&1; rc=$?
assert "9a forensics rc 0 (best-effort)" "$rc"

# --- Step 10: teardown (archive _explore out of the live topic dir) ---------
TD="$($CS explore teardown "$TOPIC")"; rc=$?
assert "10a teardown rc 0" "$rc"
assert "10b teardown stdout names the _explore archive dest" \
  "$(printf '%s' "$TD" | grep -q '_explore' && echo 0 || echo 1)"
ARCH="$(printf '%s\n' "$TD" | grep -oE '/[^ ]*/_explore[^ ]*' | head -1)"
...
$CS explore handoff-extract "$ARCH" >/dev/null 2>&1; rc=$?
assert "11a handoff-extract rc 0" "$rc"
KV="$ARCH/handoff-data.kv"
assert "11b handoff-data.kv exists" "$([ -f "$KV" ] && echo 0 || echo 1)"
assert "11c kv has mode=explore" "$(grep -q '^mode=explore$' "$KV" 2>/dev/null && echo 0 || echo 1)"
