...
# Scaffold one simulated part: its _rehearsal state (state.txt + experiments dir)
# AND the standard part dir (<topicDir>/<inst>-codex/{pane.json,outbox.jsonl}) that
...
scaffold_part() { # <art> <topicDir> <inst>
  ...
  printf '%s\n' "{\"instrument\":\"$inst\",\"model\":\"codex\",\"pane_id\":\"%${RANDOM}\"}" > "$topicDir/$inst-codex/pane.json"
...
write_result() { # <art> <inst> <expId> <metricValue>
  ...
  cat > "$art/parts/$inst/experiments/$eid/result.json" <<EOF
...
OUT="$($CS rehearsal init --slug df-mnist "maximize mnist accuracy under 100k params" 2>/dev/null)"
TOPIC="$(printf '%s\n' "$OUT" | sed -n 's/^TOPIC=//p')"
ART="$(printf '%s\n' "$OUT" | sed -n 's/^ART=//p')"
...
# Simulate spawn of 2 instruments (first two from instruments.yaml = violin, viola).
INST_A=violin
INST_B=viola
printf '%s\n%s\n' "$INST_A" "$INST_B" > "$ART/parts.txt"
scaffold_part "$ART" "$topicDir" "$INST_A"
scaffold_part "$ART" "$topicDir" "$INST_B"
check "A3 simulated spawn: parts.txt + 2 part dirs + 2 outboxes" \
  "$([ -f "$ART/parts.txt" ] && [ -f "$topicDir/$INST_A-codex/outbox.jsonl" ] && [ -f "$topicDir/$INST_B-codex/outbox.jsonl" ] && echo 0 || echo 1)"

# --- Round 1 dispatch (exp-001 to each part) -------------------------------
r=0
...
check "A4 round-1 dispatch rc 0 (both parts)" "$r"

p_a="$ART/parts/$INST_A/experiments/exp-001/prompt.md"
...
st="$(cat "$ART/parts/$INST_A/state.txt")"
...
# race-guard: both parts flipped to phase=idle (current_exp_id's result.json present).
check "A11 both parts flipped to phase=idle after score" \
  "$(grep -q 'phase=idle' "$ART/parts/$INST_A/state.txt" && grep -q 'phase=idle' "$ART/parts/$INST_B/state.txt" && echo 0 || echo 1)"

# --- status-brief shows the table + floor_met=no ---------------------------
sb="$($CS rehearsal status-brief "$TOPIC" 2>/dev/null)"
check "A12 status-brief prints | Part | table" \
  "$(printf '%s' "$sb" | grep -q '| Part |' && echo 0 || echo 1)"
...
OUT="$($CS rehearsal init --slug df-plateau "maximize plateau accuracy" 2>/dev/null)"
TOPIC_B="$(printf '%s\n' "$OUT" | sed -n 's/^TOPIC=//p')"
ART_B="$(printf '%s\n' "$OUT" | sed -n 's/^ART=//p')"
topicDir_B="${ART_B%/_rehearsal}"
...
INST_P=cello
printf '%s\n' "$INST_P" > "$ART_B/parts.txt"
# scaffold_part uses the global $topicDir; point it at B's part dir explicitly.
mkdir -p "$ART_B/parts/$INST_P/experiments"
printf 'exp_counter=0\nphase=idle\ncurrent_exp_id=\nlast_event=spawn\n' > "$ART_B/parts/$INST_P/state.txt"
mkdir -p "$topicDir_B/$INST_P-codex"
printf '%s\n' "{\"instrument\":\"$INST_P\",\"model\":\"codex\",\"pane_id\":\"%9\"}" > "$topicDir_B/$INST_P-codex/pane.json"
...
EOF
  ...
  $CS rehearsal score "$TOPIC_B" >/dev/null 2>&1
  i=$((i + 1))
done
cl="$($CS rehearsal status-brief "$TOPIC_B" 2>/dev/null | grep 'Completion check' || true)"
...
# Reuse Scenario B's part (cello) — its outbox already has done lines.
# Pre-write liveness-cursor.txt=0 under partStateDir, then monitor --once.
printf '0' > "$ART_B/parts/$INST_P/liveness-cursor.txt"
OUTBOX_C="$topicDir_B/$INST_P-codex/outbox.jsonl"
SIZE_C="$(wc -c < "$OUTBOX_C")"
MON_OUT="$($CS rehearsal monitor "$TOPIC_B" "$INST_P" --once 2>/dev/null)"
# First emitted line must parse as {"part":INST,"event":"done",...}.
FIRST_LINE="$(printf '%s\n' "$MON_OUT" | head -1)"
PARSE_OK="$(printf '%s' "$FIRST_LINE" | node -e '
let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{
  try{const o=JSON.parse(s.trim());process.exit(o.part==="'"$INST_P"'"&&o.event==="done"?0:1);}catch{process.exit(1);}
});')" && PARSE_OK=0 || PARSE_OK=$?
check "C1 monitor --once prints a parseable {part,event:done} line" "$PARSE_OK"
