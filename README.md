...
`/ap:fast-explore`, `/ap:autoresearch` and the rest in a Claude Code session; that session briefs the
workers, verifies what they produce, and finishes the run.
...
`codex` on the repository's code) research in parallel, cross-verify each other's solo claims, and
the hub writes one landscape doc and the handoff. No confidence gate, no adversary round, no
framing or grill interview — the only questions are a worker's own and the one-time forensics
consent, and a run is minutes rather than an hour.

- You get the same two artifacts in the same archived run dir: `landscape-<date>-<topic>.md` (with
  an explicit "adversary phase not run" note) and `design-handoff.md`, whose `mode=` reads
  `fast-explore`. The Conclusion is printed to chat.
- State, archive and forensics are explore's — `/ap:list`, `/ap:stop` and `/ap:review` see it as an
  explore run.
...
- Every path the doc's Components section cites must exist in the checkout: the design audit (and
...
Every command files its **forensics** at teardown (spawn failures, the hub's flags against ap and
its reflection) as an issue on the ap tracker, queued locally while
...
prefix a single command. They survive plugin updates.
...
~/.ap/                               # GLOBAL, survives teardown
  archive/<repo-hash>/<topic>/…      # archived workers + run dirs (_quick-<ts>, _design-<ts>, …)
  forensics/queue/…                  # records waiting to be filed as issues (+ map.txt run -> issue)
  issues-consent                     # your one-time yes/no for the tracker
