// Per-field consensus across workers' latest-ok results. Faithful to
...
/** latestOk: agent -> field map (its latest ok result.json fields). */
export function buildConsensus(latestOk: Record<string, Record<string, unknown>>, opts: ConsensusOpts): string {
  ...
  for (const f of FIELDS) {
    ...
    if (miss === agents.length) { missing.push(`- ${f}`); continue; }
    ...
    if (allAgree) {
      ...
      let row = `| ${f}`;
      for (const inst of agents) row += ` | ${field(inst, f) || "—"}`;
      contested.push(`${row} |`);
    }
  }
  ...
  if (contested.length) {
    ...
    for (const inst of agents) { header += ` | ${inst}'s value`; sep += "|---"; }
