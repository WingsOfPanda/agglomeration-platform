// Pure handoff logic for /ap:autoresearch. Faithful to deep-research-handoff-extract.sh
// + the extract-handoff-data helper. Winner source = scoreboard.md first-ok row.

export interface ScoreboardRow { rank: string; expId: string; agent: string; metric: string; status: string; }
...
export function parseScoreboard(md: string): { rows: ScoreboardRow[]; winner: ScoreboardRow | null; runnerUps: ScoreboardRow[] } {
  ...
  for (const line of md.split("\n")) {
    ...
    // c[0]="" c[1]=rank c[2]=exp c[3]=agent c[4]=metric c[5]=status
...
export interface HandoffInput {
  ...
  generatedTs: string;
  winner: null | {
    agent: string; exp: string; approach: string; metric: string;
    checkpoint?: string; notes?: string; codeDir: string;
  };
  runnerUps: { agent: string; exp: string; metric: string; approach: string }[];
}
...
export function buildHandoffKv(i: HandoffInput): string {
  ...
  if (i.landscapeDoc) L.push(`landscape_doc=${i.landscapeDoc}`);
  L.push(`winner_agent=${w.agent}`, `winner_exp=${w.exp}`, `winner_approach=${w.approach || "unknown"}`,
