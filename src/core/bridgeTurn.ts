// src/core/bridgeTurn.ts — round-1 brief + round-N follow-up builders for /ap:bridge.
// Like turn.ts's composers, these bodies do NOT carry a done-event line or END_OF_INSTRUCTION;
// inboxWrite appends exactly one of each (the explore duplicate-END_OF_INSTRUCTION lesson, 0.1.25).
...
export function composeBridgeBrief(task: string, repoPath: string, branch: string): string {
  return [
...
export function composeBridgeFollowup(text: string, round: number): string {
  return [
