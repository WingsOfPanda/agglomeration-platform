...
Conclusion is the hand-off seed for `/ap:score`, emitted as `score-handoff.md`. **The Hub
...
rows are intentionally absent (N varies 2 or 3); each `[workers]` row covers the whole roster in
parallel.

| # | subject | activeForm |
|---|---|---|
| 0   | `0 Args + init + roster [hub]`         | `Staging args` |
...
```
$CS prelude research-send <TOPIC> <agent> <provider>
```
...
```
Bash(command='$CS prelude research-wait <TOPIC> <agent> <provider>', run_in_background: true,
     description='prelude research-wait <agent>')
```
...
```
$CS prelude adversary-send <TOPIC> <agent> <provider>
```
...
```
Bash(command='$CS prelude adversary-wait <TOPIC> <agent> <provider>', run_in_background: true,
     description='prelude adversary-wait <agent>')
```

**Do not proceed until `$CS prelude wait-gate <TOPIC> adversary` exits 0** — it prints `<INST>\t<terminal|question|pending>` per worker; rc 1 means some worker is still `pending`/`question`, so keep handling / relay and re-run. Only on rc 0 continue. The `AS=` value is
...
1. **Pane teardown first.** Read the roster agents and run
   `$CS coda --pairs <TOPIC> <agent…>` — one 9s graceful **DONE**-banner batch across all panes
...
## Phase 9c — compose score-handoff.md (Hub Writes)

Read `$ART/handoff-data.kv` (the mechanical facts) AND the landscape doc it names via
`landscape_doc=`. As Hub, **use the Write tool** to author `$ART/score-handoff.md` with this
